#!/usr/bin/env python3
"""
逆M水滴 LBM v9 — 摆动入流 Oscillating Inflow
v8→v9核心改进：左边界入流不再是直线，而是上下正弦摆动
→ 产生更丰富的非定常涡街，尾流交叉卷曲更美
"""
...
# ═══════ 摆放入流参数 ═══════
# 核心改动: 入流在垂直方向正弦摆动
SWAY_AMPLITUDE = 0.012     # 垂直速度幅值 (u0=0.04的30%)
SWAY_PERIOD    = 400       # 摆动周期(步) — 约10个周期覆盖4000步
SWAY_PHASE     = 0.0       # 初始相位
...
# ═══════ 仿真参数 ═══════
NX, NY = 350, 175
u0 = 0.04; tau = 0.60; omega = 1.0/tau
scale = 125.0 / 5.333
...
print(f'╔══════════════════════════════════════╗')
print(f'║  逆M水滴 LBM v9 — 摆动入流          ║')
print(f'║  N={NX}×{NY}  u0={u0}  τ={tau}  Re≈{Re_est:.0f}  ║')
print(f'║  摆幅={SWAY_AMPLITUDE}  周期={SWAY_PERIOD}步   ║')
print(f'║  帧数={STEPS//SAVE_EVERY}  输出={OUT_DIR}       ║')
print(f'╚══════════════════════════════════════╝')
...
def anim_vort(i):
    ...
    ax.set_title(f'逆M水滴绕流 v9 Re≈{Re_est:.0f} t={i*SAVE_EVERY} '
                 f'[入流摆动±{SWAY_AMPLITUDE*1000:.0f}‰]',
                 fontsize=12, fontweight='bold')
    ax.set_xlim(0, NX); ax.set_ylim(0, NY)

ani = animation.FuncAnimation(fig, anim_vort, frames=len(frames), interval=80)
gif_vort = os.path.join(OUT_DIR, 'droplet_MRT_v9_sway_vorticity.gif')
ani.save(gif_vort, writer='pillow', fps=12, dpi=80); plt.close(fig)
print(f'✅ {gif_vort} ({os.path.getsize(gif_vort)//1024}KB)')

# ═══════ 渲染 — 涡街版 (coolwarm) ═══════
print(f'渲染 {len(frames)} 帧 — 涡街版…')
...
def anim_vortex(i):
    ...
    ax2.set_title(f'逆M水滴涡街 v9 Re≈{Re_est:.0f} frame={i} '
                  f'[摆动入流]', fontsize=12)
    ax2.set_xlim(0, NX); ax2.set_ylim(0, NY)

ani2 = animation.FuncAnimation(fig2, anim_vortex, frames=len(frames), interval=80)
gif_vortex = os.path.join(OUT_DIR, 'droplet_MRT_v9_sway_vortex.gif')
ani2.save(gif_vortex, writer='pillow', fps=12, dpi=80); plt.close(fig2)
