...
print(f'LBM v12 fp32 coarse  N={NX}x{NY}  Re~{Re}')
print(f'sway={SWAY_A}  T={SWAY_T}  steps={STEPS}  fps={FPS}')
...
vm=abs(frames[-1]).max()*0.15  # was 0.3, now 0.15 = 2x more saturated
def av(i):
    ...
    ax.set_title(f'Inverse-M Droplet v12 Re~{Re} t={tp} [sway x7 24fps]',fontsize=12,fontweight='bold')
    ax.set_xlim(0,NX); ax.set_ylim(0,NY)
ani=animation.FuncAnimation(fig,av,frames=len(frames),interval=80)
g1=os.path.join(OUT_DIR,'droplet_v12_coarse_vorticity.gif')
ani.save(g1,writer='pillow',fps=FPS,dpi=80); plt.close(fig)
print(f'OK {g1} ({os.path.getsize(g1)//1024}KB)')

fig2,ax2=plt.subplots(figsize=(10,5))
vm2=abs(frames[-1]).max()*0.12  # even more saturation for vortex
def av2(i):
    ...
    ax2.set_title(f'Inverse-M Vortex v12 Re~{Re} f={i} [sway x7 24fps]',fontsize=12)
    ax2.set_xlim(0,NX); ax2.set_ylim(0,NY)
ani2=animation.FuncAnimation(fig2,av2,frames=len(frames),interval=80)
g2=os.path.join(OUT_DIR,'droplet_v12_coarse_vortex.gif')
