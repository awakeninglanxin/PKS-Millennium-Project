...
# P0: 心形×5深度层, 每层半径级联递减
#   expand=1.12/1.30/1.55/1.80/2.10 → level=0/1/2/3/4
DEPTH_CONFIG = [(1.12, 0), (1.30, 1), (1.55, 2), (1.80, 3), (2.10, 4)]
for i in range(91):
    th = math.pi * i / 90
    c_att = 0.5*cmath.exp(1j*th) - 0.25*cmath.exp(2j*th)
    for expand, lvl in DEPTH_CONFIG:
        c_out = c_att * expand
        if abs(c_out) < 1e-12: continue
        c_inv = 1.0/c_out
        if abs(c_inv.real) < 6 and abs(c_inv.imag) < 5:
            upper_sources.append((c_inv.real, abs(c_inv.imag), lvl))
...
print(f"  {n_up}个上半源(91×5深度层+泡) → {n_total}个(含镜像)")
...
out=os.path.join(OUT_DIR,"逆M_P36_递归9层_v2.png")
img_pil.save(out)
