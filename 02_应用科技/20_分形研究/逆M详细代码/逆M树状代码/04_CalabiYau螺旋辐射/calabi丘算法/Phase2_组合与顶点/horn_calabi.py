...
slices_values = [15] * len(n_vs)  # 定义 slices_values 列表，每个元素为 5
...
filename = f'calabi-Yau horn_{len(n_vs)}图合一{alpha_v}_2^n.obj'  # 定义输出文件名

def calabi_yau(z, k1, k2, alpha, n):
    exp_k1 = np.exp(2 * np.pi * (1j) * k1 / n)  # 计算 exp_k1
    exp_k2 = np.exp(2 * np.pi * (1j) * k2 / n)  # 计算 exp_k2
    z1 = exp_k1 * (np.cos(z) ** (2 / n))  # 计算 z1
    z2 = exp_k2 * (np.sin(z) ** (2 / n))  # 计算 z2
