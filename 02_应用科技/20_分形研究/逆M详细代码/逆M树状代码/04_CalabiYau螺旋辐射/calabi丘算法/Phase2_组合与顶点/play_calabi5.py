...
n = 12
slices = 12
angles = [0,np.pi]  # 3个角度，分别对应每个图形的位置
filename = f'play_3d图_颜色_{n}极_{slices}{angles}片_8图形_双色渐变.obj'
        ...
        for k2 in range(n):
            for i in range(slices - 1):
                for j in range(slices - 1):
                    ...
                    color_u = cm.get_cmap('spring')((u[i] + 1) / 2)  # Normalize u to [0, 1] range
                    color_v = cm.get_cmap('spring_r')(v[j] / (np.pi / 2))  # Normalize v to [0, 1] range
...
def create_mesh():
    ...
    all_faces = []
    all_colors = []
    offset_step = 0  # Offset step to separate the 8 models
