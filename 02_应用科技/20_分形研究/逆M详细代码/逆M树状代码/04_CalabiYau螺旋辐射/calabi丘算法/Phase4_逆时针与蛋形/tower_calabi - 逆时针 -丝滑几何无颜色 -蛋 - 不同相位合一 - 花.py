...
# 创建4个从0到π的均匀相位值
alpha_list = np.linspace(0, np.pi, 4)
...
# 为每个相位值创建网格
for idx, alpha_v in enumerate(alpha_list):
    # 创建所有网格
    positions = []
    current_height = 0
    all_meshes = []
    ...
    for n, slices, scale in zip(n_values, slices_values, scale_factors):
        mesh = create_mesh(n, slices, (0, current_height, 0), scale, alpha_v)
        bounds = mesh.bounds
    ...
    # 生成文件名，包含相位值信息
    filename = f'calabi-yau-蛋-花_{len(n_vs)}_layers_phase{idx+1}_alpha{alpha_v:.2f}.obj'
    ...
    print(f"Normalization scale for phase {idx+1} (x, y, z): {normalized_scale}")