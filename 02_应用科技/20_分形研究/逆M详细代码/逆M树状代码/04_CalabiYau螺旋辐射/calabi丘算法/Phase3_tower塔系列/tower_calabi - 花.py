...
slices_values = [8] * len(n_vs)  # 定义 slices_values 列表，每个元素为 6
...
def calabi_yau(z, k1, k2, alpha, n):
    exp_k1 = np.exp(2 * np.pi * (1j) * k1 / n) *(1-np.sin(z))# 计算 exp_k1
    exp_k2 = np.exp(2 * np.pi * (1j) * k2 / n) *(1-np.sin(z))# 计算 exp_k2
