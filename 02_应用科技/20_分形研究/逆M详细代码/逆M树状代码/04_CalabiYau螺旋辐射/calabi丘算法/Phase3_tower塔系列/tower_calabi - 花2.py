...
scale_factors = [2 ** (factor / 2) for factor in range(len(n_vs) - 1, -1, -1)]
alpha_v = 0
filename = f'calabi-Yau超美3d_花2_{len(n_vs)}图合一{alpha_v}_2^n.obj'
thickness = 0.01  # 面的厚度
...
for n, slices, scale in zip(n_values, slices_values, scale_factors):
    ...
    disk = create_disk(radius=0.5 * scale, thickness=0.02, position=disk_position)
...
combined_with_cylinder_and_disk.export(filename)
print(f"Saved: {filename}")

# 保存为 STL 格式
filename_stl = filename.replace('.obj', '.stl')
