...
# ============================================================
# 3. ★ 核心：距原点距离 → 帕斯卡行号 ★
#    三角顶点从原点(0,0)出发→向外层层渲染
# ============================================================
print("[3/5] 距原点距离 → Pascal row ...")

# ★ 改为用距原点(0,0)的距离决定帕斯卡行号 n ★
# - 原点(0,0)处 n=0（三角顶点，最密）
# - 水滴边缘处 n=N-1（三角底部，最疏）
# - 从原点向水滴边缘逐层展开
...
# 水滴内部归一化距离
r_valid = dist_origin[inside_droplet]
r_min = r_valid.min()
r_max = r_valid.max()
r_norm = np.clip((dist_origin - r_min) / (r_max - r_min + 1e-12), 0, 1)
print(f"  距原点距离: [{r_min:.3f}, {r_max:.3f}]")

# n = r_norm^D * (N-1)
# 原点 r_norm≈0 → n≈0（三角顶点）
# 边缘 r_norm≈1 → n≈N-1（三角底部）
...
# 距离映射：近→n小(三角顶)，远→n大(三角底)
r_sample = np.abs(proj_sample)
r_s_min = max(r_sample.min(), r_min)
r_s_max = min(r_sample.max(), r_max)
r_s_norm = np.clip((r_sample - r_s_min) / (r_s_max - r_s_min + 1e-12), 0, 1)
n_sample = (r_s_norm ** D) * (N_ROWS - 1)
ax.plot(proj_sample, n_sample, 'y-', linewidth=2.5)
ax.set_title(f"C. Distance from origin → n mapping\n"
             f"n = (r_norm)^{D:.3f} x {N_ROWS-1}\n"
             f"Origin(0,0) → n=0 (triangle tip) | Edge → n=N-1 (triangle base)",
             fontsize=12)
ax.set_xlabel("Distance from origin (along droplet axis)", fontsize=10)
...
# 往外：红→橙→黄→绿→蓝→紫
hue += 0.7 * n_vis * np.clip(iter_count / MAX_ITER, 0, 1)
...
for ax, p_t in zip(axes, [2, 3, 5]):
    ...
    # ★ 距原点距离 → n (同主图)
    ni = np.clip(((r_norm ** Dt) * (NR - 1)).astype(np.int64), 0, NR-1)
...
fig.suptitle(
    ...
    f"Shape = iter_count (preserved) | Triangle tip at origin (0,0)\n"
    f"n = r_norm^D · N  |  k = (θ+π)/(2π)·n",
...
│                                                                        │
│  ④ n(r) = (r_norm)^D · (N−1)                                          │
│     Maps distance from origin → Pascal row → C_iter_to_n.png           │
│     原点(0,0)=三角顶点 → 边缘=三角底部                                   │
│                                                                        │
│  ⑤ ★ 360° 全方向映射, 原点 (0,0) ★                                    │
│     k = (θ+π)/(2π) · n   (无张角限制)                                  │
...
│                                                                        │
│  ⑥ brightness = pascal[ n(r) ][ k(θ) ]                                │
│     Lookup only, no multiplication                                    │
│     → D_droplet_gray.png (triangle vertex at origin (0,0))              │
│     → E_droplet_blue.png (layered outward from origin)                  │
│                                                                        │
