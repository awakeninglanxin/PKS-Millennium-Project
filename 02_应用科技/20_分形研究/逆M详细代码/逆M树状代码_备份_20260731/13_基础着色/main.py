...
z=np.zeros_like(ce);alive=np.ones(ce.shape,bool);esc=np.full(ce.shape,MI)
for n in range(MI):
    if not alive.any():break
    idx=np.where(alive);za=z[idx].copy();ca=ce[idx].copy();za=za**2+ca;z[idx]=za
    new=alive&(np.abs(z)>BL);esc[new]=n;alive&=~new
interior=np.rot90(alive,k=3);esc=np.rot90(esc,k=3)
sv=esc[~interior]
if len(sv)>1:
    lo,hi=np.percentile(sv,[5,95])
    norm=np.clip((esc-lo)/(hi-lo+1e-10),0,1)
else:
    norm=esc*0
cmap=matplotlib.colormaps['viridis'];img=cmap(norm)[...,:3]
img_flat=img.reshape(-1,3);img_flat[interior.ravel()]=[0,0,0]
fig,ax=plt.subplots(figsize=(8,8*H/W),dpi=150)
ax.imshow(img,extent=[I0,I1,R0,R1],origin='lower',interpolation='nearest')
ax.axis('off');plt.tight_layout(pad=0)
plt.savefig(os.path.join(od,"UF13_基础着色.png"),dpi=200,bbox_inches='tight',facecolor='black');plt.close()
print("UF13 done")
