...
# -*- coding: utf-8 -*-
"""
UF21 v7 — 蓝白阴阳格水滴分形（精调终版）

v6→v7 微调：
  1. ★ 金边大幅增强：更厚(24px bloom) + 更亮(峰值接近纯白) + 双色渐变(内白金→外橙红)
  2. 内部同心环柔化：增加插值段数，降低对比度
  3. 外部基底微提亮：整体+0.03~0.05 通道值
  4. 棋盘格alpha降至0.15（极致朦胧感）
  5. 新增：内部微小蓝色环境光（防止纯白过曝）
"""
...
N_ANGLE=440;LOG_R_STEP=0.026;CHESS_ALPHA=0.15;CHESS_BLUR=1.2
...
GOLD_WHITE =[1.0,1.0,0.70]     # 内侧：白金色（极亮）
GOLD_HOT   =[1.0,0.88,0.25]     # 热：金黄色
GOLD_WARM  =[0.95,0.55,0.06]     # 暖：橙色
GOLD_EMBER=[0.75,0.28,0.02]     # 余烬：暗红褐
DEM_SCALE=25.0
...
def render(data,w,h):
    ...
    with np.errstate(invalid='ignore',divide='ignore'):
        ...
        def lut(v,r0,g0,b0,r1,g1,b1,r2,g2,b2,r_lim,g_lim,b_lim):
            r=np.select([v<0.25,v<0.55,v<0.82,True],
                [r0+(r1-r0)*(v/0.25),r1+(r2-r1)*((v-0.25)/0.30),
                 r2+(r_lim-r2)*((v-0.55)/0.27),r_lim])
            g=np.select([v<0.25,v<0.55,v<0.82,True],
                [g0+(g1-g0)*(v/0.25),g1+(g2-g1)*((v-0.25)/0.30),
                 g2+(g_lim-g2)*((v-0.55)/0.27),g_lim])
            b=np.select([v<0.25,v<0.55,v<0.82,True],
                [b0+(b1-b0)*(v/0.25),b1+(b2-b1)*((v-0.25)/0.30),
                 b2+(b_lim-b2)*((v-0.55)/0.27),b_lim])
            return np.stack([r,g,b],axis=-1)

        ext_rgb=lut(ts,
            EXT_FAR[0],EXT_FAR[1],EXT_FAR[2],
            EXT_MID[0],EXT_MID[1],EXT_MID[2],
    ...
    with np.errstate(invalid='ignore'):
        ...
        # 5段：hot→core→mid→edge→deep
        r_i=np.select([v_adj<0.12,v_adj<0.30,v_adj<0.55,v_adj<0.80,True],
            [INT_HOT[0]+(INT_CORE[0]-INT_HOT[0])*(v_adj/0.12),
             INT_CORE[0]+(INT_MID[0]-INT_CORE[0])*((v_adj-0.12)/0.18),
             INT_MID[0]+(INT_EDGE[0]-INT_MID[0])*((v_adj-0.30)/0.25),
             INT_EDGE[0]+(INT_EDGE[0]*0.65-INT_EDGE[0])*((v_adj-0.55)/0.25),
             INT_EDGE[0]*0.65*(1-(v_adj-0.80)/0.20)])
        g_i=np.select([v_adj<0.12,v_adj<0.30,v_adj<0.55,v_adj<0.80,True],
            [INT_HOT[1]+(INT_CORE[1]-INT_HOT[1])*(v_adj/0.12),
             INT_CORE[1]+(INT_MID[1]-INT_CORE[1])*((v_adj-0.12)/0.18),
             INT_MID[1]+(INT_EDGE[1]-INT_MID[1])*((v_adj-0.30)/0.25),
             INT_EDGE[1]+(INT_EDGE[1]*0.68-INT_EDGE[1])*((v_adj-0.55)/0.25),
             INT_EDGE[1]*0.68*(1-(v_adj-0.80)/0.20)])
        b_i=np.select([v_adj<0.12,v_adj<0.30,v_adj<0.55,v_adj<0.80,True],
            [INT_HOT[2]+(INT_CORE[2]-INT_HOT[2])*(v_adj/0.12),
             INT_CORE[2]+(INT_MID[2]-INT_CORE[2])*((v_adj-0.12)/0.18),
             INT_MID[2]+(INT_EDGE[2]-INT_MID[2])*((v_adj-0.30)/0.25),
             INT_EDGE[2]+(INT_EDGE[2]*0.75-INT_EDGE[2])*((v_adj-0.55)/0.25),
    ...
    # 5层辉光
    g_ember=binary_dilation(edge,np.ones((28,28)))   # 最外层余烬
    g_warm=binary_dilation(edge,np.ones((18,18)))   # 暖橙层
    g_gold=binary_dilation(edge,np.ones((10,10)))   # 金色层
    g_hot =binary_dilation(edge,np.ones((5,5)))    # 热层
    g_core=binary_dilation(edge,np.ones((2,2)))     # 核心白金
    ...
    if ge.any():
        d=np.power(dn[ge],2.0)
        img[ge,0]=np.clip(img[ge,0]+0.10*d,0,1)
        img[ge,1]=np.clip(img[ge,1]+0.05*d,0,1)
        img[ge,2]=np.clip(img[ge,2]+0.01*d,0,1)
    ...
    if gw.any():
        d=dn[gw]
        img[gw]=np.stack([
            np.clip(GOLD_EMBER[0]*0.35+GOLD_WARM[0]*0.60*d,0,1),
            np.clip(GOLD_EMBER[1]*0.25+GOLD_WARM[1]*0.68*d,0,1),
            np.clip(GOLD_EMBER[2]*0.08+GOLD_WARM[2]*0.85*d,0,0.32)],axis=-1)
    ...
    if gg.any():
        d=dn[gg]
        img[gg]=np.stack([
            np.clip(GOLD_WARM[0]*0.40+GOLD_HOT[0]*0.56*d,0,1),
            np.clip(GOLD_WARM[1]*0.32+GOLD_HOT[1]*0.64*d,0,1),
            np.clip(GOLD_WARM[2]*0.12+GOLD_HOT[2]*0.82*d,0,0.40)],axis=-1)
    ...
    if gh.any():
        d=dn[gh]
        img[gh]=np.stack([
            np.clip(GOLD_HOT[0]*0.55+GOLD_WHITE[0]*0.42*d,0,1),
            np.clip(GOLD_HOT[1]*0.48+GOLD_WHITE[1]*0.48*d,0,1),
            np.clip(GOLD_HOT[2]*0.18+GOLD_WHITE[2]*0.75*d,0,0.50)],axis=-1)
    ...
    if gc.any():
        d=dn[gc]
        img[gc]=np.stack([
            np.clip(GOLD_WHITE[0]*0.62+0.36*d,0,1),
            np.clip(GOLD_WHITE[1]*0.58+0.39*d,0,1),
            np.clip(GOLD_WHITE[2]*0.35+0.60*d,0,0.58)],axis=-1)
...
if __name__=='__main__':
    ...
    print(f"=== v7 精调终版 === {WIDTH}x{HEIGHT}")
    ...
    out=os.path.join(out_dir,'最美的蓝色水滴分形_v7.png')
