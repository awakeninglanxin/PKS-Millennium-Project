#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""UF17 指数平滑 (Exponential Smoothing) — 势能指数映射"""
...
# 指数平滑: exp(-smooth) 产生快速衰减颜色带
exp_val=np.exp(-sm*0.3)
sv=exp_val[~interior];lo,hi=np.percentile(sv,[2,98]) if len(sv)>0 else (0,1)
norm=np.clip((exp_val-lo)/(hi-lo+1e-10),0,1)
cmap=matplotlib.colormaps['twilight'];img=cmap(norm)[...,:3]
...
plt.savefig(os.path.join(od,"UF17_指数平滑.png"),dpi=200,bbox_inches='tight',facecolor='black');plt.close()
print("UF17: Exponential Smoothing done")
