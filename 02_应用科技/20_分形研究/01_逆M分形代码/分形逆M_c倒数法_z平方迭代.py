...
@jit(nopython=True)
def compute_inverse_mandelbrot(xmin, xmax, ymin, ymax, width, height, max_iter):
...
class InverseMandelbrotViewer:
    def __init__(self):
        # 视图历史和初始参数
        self.view_stack = []
        self.max_iter = 1024  # 与C++代码的NUMITR一致
        self.base_iter = 1024
        self.width, self.height = 800, 600

        # 初始视图范围 - 调整为与C++代码相同的范围
        self.xmin, self.xmax = -1.5, 4.25
        self.ymin, self.ymax = -2.875, 2.875

        # 创建图形界面
        self.fig, self.ax = plt.subplots(figsize=(12, 8))

        # 使用'hot'色彩映射 - 类似C++代码中的csCColdeFireRamp
        self.img = self.ax.imshow(
            np.zeros((self.height, self.width)),
            cmap='hsv',  # 使用hot色彩映射，类似火焰色
    ...
    def update_title(self):
        zoom_factor = (5.75 / (self.xmax - self.xmin)) ** 0.5
        self.ax.set_title(
            f"Inverse Mandelbrot Set | Iter: {self.max_iter} | Zoom: {zoom_factor:.1f}x\n"
    ...
    def update_plot(self):
        ...
        divergence = compute_inverse_mandelbrot(
            self.xmin, self.xmax,
            self.ymin, self.ymax,
        ...
        # 调整颜色映射范围，count*15对应C++代码中的处理
        self.img.set_norm(colors.PowerNorm(0.5, vmin=0, vmax=self.max_iter * 0.15))
        self.update_title()
    ...
    def on_select(self, eclick, erelease):
        ...
        # 动态增加迭代次数（基于缩放级别）
        zoom_factor = (5.75 / (self.xmax - self.xmin))
