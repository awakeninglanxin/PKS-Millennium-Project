...
- **双层高度约束**（k1 比例 + k2 间隙，二选一）+ **双软约束提醒**（橙/绿色信息栏）
- **📷一键截长图**（html2canvas 2倍高清导出 PNG，含全页面）
- **🌀双色螺旋线**（🟠中心线 + 🔵边沿线，三种曲线模式，独立渐隐拖尾 + 可调尾迹长度）
- **📐滚筒摆线轨迹**（实时累积滚筒边缘散点，子采样比例匹配转速，永不消失）
- **行星齿轮自转**（锁定=自动 ratio=statorPoles/rollerPoles，解锁=各环手动滑条）
- **抛物线加速**（speed 从 0 爬升，ratio 从 1:1:1 慢慢过渡到 1:2.5:6.25，滑条可调速率）
