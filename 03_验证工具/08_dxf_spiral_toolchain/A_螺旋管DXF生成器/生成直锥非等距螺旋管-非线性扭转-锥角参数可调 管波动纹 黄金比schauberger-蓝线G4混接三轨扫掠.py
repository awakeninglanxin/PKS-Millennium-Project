...
class SpiralSweepGenerator:
    def __init__(self, start_dxf_path, end_dxf_path, bottom=300, high=300, n_turns=2, sign_spiral_dir=1,
                 sign=1, twist=1, amp=1, alpha_deg=11, collect_n=50, amp_nonlinear=False, dxf_points=16,
                 twist_type='linear'):
        """
        初始化螺旋扫掠生成器 - 只支持DXF文件
    ...
    def sweep_section(self):
        ...
        for i in range(self.num_t):
            ...
            for j in range(len(x_blend)):
                ...
                # 记录中点 (j=mid_index)
                if j == mid_index:
                    mid_points.append(global_point)
        ...
        return swept_sections, center_points.tolist(), egghead_points, mid_points  # 返回中点列表
    ...
    def segment_egghead_points_by_jump(self, egghead_points):
        """根据锯齿波跳变点分割蛋咬一口尖点曲线，并在段间添加混接曲线"""
        segments = []  # 存储分割后的点段
        current_segment = [egghead_points[0]]

        # 使用预计算的波形值检测跳变点
        jump_threshold = 0.5  # 波形值变化超过0.5视为跳变
        jump_points = []  # 记录跳变点位置

        # 首先对曲线进行离散化处理
        discrete_points = []
        for i in range(len(egghead_points) - 1):
            discrete_points.append(egghead_points[i])
        ...
        discrete_points.append(egghead_points[-1])
        ...
        print(f"蛋咬一口尖点曲线被分成 {len(blended_segments)} 段")
    ...
    def save_to_dxf(self, filename, turn_interval=0.5, n_end_sections=2):
        ...
        # 生成并添加所有曲线
        swept_sections, center_points, egghead_points, mid_points = self.sweep_section()  # 获取中点列表
        ...
        # 添加蛋咬一口尖线（分段并添加图层）
        segments = self.segment_egghead_points_by_jump(egghead_points)
        print(f"蛋咬一口尖点曲线分段数量: {len(segments)}")

        for idx, seg in enumerate(segments):
            print(f"分段 {idx + 1}: {len(seg)} 个点")

            # 创建带顺序编号的图层
            layer_name = f"egg_tip_segment_{idx + 1:03d}"
            if layer_name not in doc.layers:
                doc.layers.new(name=layer_name)

            if len(seg) >= 2:  # 只有两个点以上才能生成SPLINE
                spline = msp.add_spline(seg)
                spline.dxf.layer = layer_name
                spline.dxf.color = colors['egg_tip']
            elif len(seg) == 1:  # 单点添加点对象
                point = msp.add_point(seg[0])
                point.dxf.layer = layer_name
                point.dxf.color = colors['egg_tip']

        # 添加中点曲线（使用与蛋咬一口尖点相同的分段和混接逻辑）
        mid_segments = self.segment_egghead_points_by_jump(mid_points)
        print(f"中点曲线分段数量: {len(mid_segments)}")

        for idx, seg in enumerate(mid_segments):
            print(f"中点分段 {idx + 1}: {len(seg)} 个点")

            # 创建带顺序编号的图层
            layer_name = f"mid_point_segment_{idx + 1:03d}"
            if layer_name not in doc.layers:
                doc.layers.new(name=layer_name)
            ...
            if len(seg) >= 2:
                spline = msp.add_spline(seg)
                spline.dxf.layer = layer_name
                spline.dxf.color = colors['mid_point_curve']  # 使用青色
            elif len(seg) == 1:
                point = msp.add_point(seg[0])
                point.dxf.layer = layer_name
                point.dxf.color = colors['mid_point_curve']
...
if __name__ == "__main__":
    ...
    generator_right.save_to_dxf('波纹螺旋锥管 指数螺旋_phi_爱心_三轨扫掠.dxf',
