...
class SpiralSweepGenerator:
    ...
    def segment_egghead_points_by_jump(self, egghead_points):
        ...
        # 使用预计算的波形值检测跳变点
        # 锯齿波跳变特征：波形值从接近1突然降到接近0
        jump_threshold = 0.5  # 波形值变化超过0.5视为跳变

        # 调试信息：记录跳变点位置
        jump_points = []

        # 首先对曲线进行离散化处理（使用1阶贝塞尔连接）
        discrete_points = []
        for i in range(len(egghead_points) - 1):
            ...
            if abs(prev_wave - next_wave) > jump_threshold:
                # 在跳变点处添加一个中间点，使曲线断开
                discrete_points.append(None)  # 使用None标记跳变点
        ...
        # 打印分段信息
        print(f"蛋咬一口尖点曲线被分成 {len(segments)} 段")
...
if __name__ == "__main__":
    ...
    generator_right.save_to_dxf('波纹螺旋锥管 指数螺旋_phi_爱心.dxf',
