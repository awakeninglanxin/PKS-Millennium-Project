...
t = [-math.pi / 3 + i * (2 * math.pi / step) for i in range(step)]
degree_rad = 0.582

# Define the equation
d =216  # Assume d=216
b= 1.618
sqrt_term = [1 / (b - ti * math.sin(degree_rad))**2 - (ti * math.cos(degree_rad))**2 for ti in t]

# Skip the points where the value is negative and outside the valid t range
valid_indices = [i for i in range(len(sqrt_term)) if sqrt_term[i] >= 0 and -math.pi / 3 <= t[i] <= 2 * math.pi / 3]
...
curve = rs.AddInterpCurve(points,1)
