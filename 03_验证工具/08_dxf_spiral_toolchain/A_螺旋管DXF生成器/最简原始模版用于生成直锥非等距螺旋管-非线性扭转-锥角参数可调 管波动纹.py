...
class SpiralSweepGenerator:
            ...
            for entity in msp:
                if entity.dxftype() == 'SPLINE':
                    ...
                    if section_type == 'start':
                        ...
                        self.circle_x, self.circle_y = x_norm, y_norm
                        section_name = "结束断面"
    ...
    def sweep_section(self):
        ...
        # 确保断面数据是NumPy数组
        if not isinstance(self.circle_x, np.ndarray):
            self.circle_x = np.array(self.circle_x)
            self.circle_y = np.array(self.circle_y)
        if not isinstance(self.start_x, np.ndarray):
            self.start_x = np.array(self.start_x)
            self.start_y = np.array(self.start_y)

        # 确保两个断面点数相同
        min_points = min(len(self.circle_x), len(self.start_x))
        circle_x = self.circle_x[:min_points]
        circle_y = self.circle_y[:min_points]
        start_x = self.start_x[:min_points]
        start_y = self.start_y[:min_points]

        # 计算中间点索引
        mid_index = int(min_points / 2)

        for i in range(self.num_t):
            section_points = []
            t = i / (self.num_t - 1)  # 插值权重，从0到1

            # 交换了混合顺序：start_x -> circle_x
            x_blend = ((1 - t) * start_x + t * circle_x)
            y_blend = ((1 - t) * start_y + t * circle_y)
    ...
    def _precompute_exp_wave(self):
        ...
        def objective(k):
            ...
            freq = np.exp(k * t_samples) / (2 * np.pi)
            # 使用 trapezoid 替代 trapz
            phase = np.trapezoid(freq, t_samples)
            return abs(phase - target_bands +0.1)  # -0.001是为了消除末尾振幅的扩展
        ...
        # 计算频率和相位
        freq = np.exp(k * t_normalized * (self.n_turns * 2 * np.pi)) / (2 * np.pi)
        ...
        if self.amp_nonlinear:
            amplitude = np.power(2, t_normalized * (self.n_turns * 2 * np.pi) / 12)
        else:
...
if __name__ == "__main__":
    ...
    generator_right.save_to_dxf('波纹螺旋锥管 指数螺旋_固定参数.dxf',
