...
# Add dashed red lines to divide the concentric circles into 360 parts
angles = np.linspace(0, 2 * np.pi, 360, endpoint=False)
for angle in angles:
    x_end = 18*np.cos(angle)
    y_end = 18*np.sin(angle)
    plt.plot([0, x_end], [0, y_end], 'r--', linewidth=0.2)  # Red dashed line from center to the outermost circle

# Add 12 evenly spaced concentric circles between radius 8 and 3
radii = np.linspace(6, 18, 5)
for radius in radii:
    circle = plt.Circle((0, 0), radius, linestyle='--', fill=False, edgecolor='green', linewidth=0.8)
    plt.gca().add_patch(circle)

# Additional plot settings
plt.xlabel('X')
plt.ylabel('Y')
plt.title('2D Parametric Curve with Regular Polygons Scaled by π and 360 Dividing Lines')
plt.axis('equal')
plt.legend()

# Save the plot as PNG with transparent background
plt.savefig("2d_spiral_with_polygons_scaled_by_pi_and_360_divisions.png", transparent=True)
plt.show()
