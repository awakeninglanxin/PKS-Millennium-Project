...
class SpiralSweepGenerator:
    def __init__(self, spline_data_path, t_min=5 * np.pi, t_max=11 * np.pi, d=2.5, z_v=10, amp=1, sign_spiral_dir=1,
                 sign=1, twist=1):
        """
    ...
    def update_psi(self):
        ...
        exponential_growth = np.exp(2*t_normalized) - 1

        # 归一化到0到1的范围
        exponential_normalized = exponential_growth / (np.exp(2*1) - 1)
    ...
    def calculate_spiral_coords(self):
        ...
        z = (self.t_spiral - self.t_min) * self.z_v
        return x, y, z
...
if __name__ == "__main__":
    ...
    generator_right.r = -1
    generator_right.twist = -1.5  # 黄金比的周期
