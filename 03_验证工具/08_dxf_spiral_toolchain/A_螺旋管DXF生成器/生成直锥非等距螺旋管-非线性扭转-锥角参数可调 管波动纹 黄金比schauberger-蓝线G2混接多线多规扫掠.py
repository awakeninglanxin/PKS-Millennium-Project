...
class SpiralSweepGenerator:
    def __init__(self, start_dxf_path, end_dxf_path, bottom=300, high=300, n_turns=2, sign_spiral_dir=1,
                 sign=1, twist=1, amp=1, alpha_deg=11, collect_n=50,global_a=1,amp_nonlinear=False, dxf_points=16,
                 twist_type='linear'):
        """
        初始化螺旋扫掠生成器 - 只支持DXF文件
    ...
    def sweep_section(self):
        ...
        for i in range(self.num_t):
            ...
            for j in range(len(x_blend)):
                ...
                vector = self.exp_wave(i) * (R_theta @ (N_term1 + B_term1))
                global_point = [
    ...
    def find_closest_points_to_z_axis(self, swept_sections, guide_points, turn_interval=0.5, n_end_sections=2):
        """
        找到每指定圈数间隔范围内距离z轴最近的点，以及首尾断面的最近点
        增加内插值曲线功能（颜色2）
        :param swept_sections: 扫掠断面列表
        :param guide_points: 导轨点列表
        ...
        # 计算每个点到z轴的距离
        distances = [np.sqrt(p[0] ** 2 + p[1] ** 2) for p in guide_points]
        ...
        for i in range(num_intervals):
            ...
            # 收集点
            collected_points = guide_points[start_collect:end_collect]
    ...
    def create_scale_invariant_blend(self, p_prev2, p_prev, p0, p1, p2, p3,
                                     tension=0.6, curvature_strength=0.15, samples=30):
        """
        尺度不变的五次 Bézier 混接：
        - 端点 G² 连续（匹配一阶/二阶导数）
        - 仅用 6 个已存在的点：上一段末的 p_prev2, p_prev, p0；下一段首的 p1, p2, p3
        - 使用归一化导数，并让导数目标的尺度随 dist^1 / dist^2 变化，从而在 global_a 缩放时形态不变
        """
        # 转数组
        p_prev2 = np.array(p_prev2, dtype=float)
        p_prev = np.array(p_prev, dtype=float)
        ...
        # --- 端点两侧的“步长”估计（用于归一化有限差分） ---
        h_in = max(1e-9, 0.5 * (np.linalg.norm(p0 - p_prev) + np.linalg.norm(p_prev - p_prev2)))
        h_out = max(1e-9, 0.5 * (np.linalg.norm(p2 - p1) + np.linalg.norm(p3 - p2)))

        # --- 归一化导数近似（维度正确） ---
        # 一阶（切向）：
        T_in = (p0 - p_prev) / (np.linalg.norm(p0 - p_prev) + 1e-12)
        T_out = (p2 - p1) / (np.linalg.norm(p2 - p1) + 1e-12)  # 这里用 p1->p2 的方向

        # 二阶（曲率向量近似）：二阶差分 / h^2
        K_in = (p0 - 2 * p_prev + p_prev2) / (h_in ** 2)
        K_out = (p3 - 2 * p2 + p1) / (h_out ** 2)  # 近似 p1 一侧的“外侧曲率”

        # --- 目标端点导数的“尺度”设定（关键：随 dist 的幂次） ---
        L1 = tension * dist  # 一阶导的尺度 ∝ dist^1
        L2 = curvature_strength * (dist ** 2)  # 二阶导的尺度 ∝ dist^2

        B0 = p0
        B5 = p1

        B1 = B0 + (L1 / 5.0) * T_in
        B2 = (L2 / 20.0) * K_in + 2 * B1 - B0

        B4 = B5 - (L1 / 5.0) * T_out
        B3 = (L2 / 20.0) * K_out + 2 * B4 - B5

        # 采样
        blend_points = []
        for t in np.linspace(0.0, 1.0, samples):
            mt = 1.0 - t
            # 五次 Bézier通式（不预计算组合数也行）
            point = (mt ** 5) * B0 \
                    + (5 * mt ** 4 * t) * B1 \
                    + (10 * mt ** 3 * t ** 2) * B2 \
                    + (10 * mt ** 2 * t ** 3) * B3 \
    ...
    def segment_egghead_points_by_jump(self, points):
        ...
        for i in range(len(segments)):
            ...
            if i < len(segments) - 1:
                ...
                # 至少要有足够的点来估计导数/曲率
                if len(prev_seg) >= 3 and len(next_seg) >= 3:
                    p_prev2 = prev_seg[-3]
                    p_prev = prev_seg[-2]
                    p0 = prev_seg[-1]
                    ...
                    # 尺度不变的五次混接
                    blend_points = self.create_scale_invariant_blend(
                        p_prev2, p_prev, p0, p1, p2, p3,
                        tension=0.6, curvature_strength=0.15, samples=30
                    )
                    blended_segments.append(blend_points)
                else:
                    # 后备：简单直线
                    blended_segments.append([segments[i][-1], segments[i + 1][0]])
    ...
    def save_to_dxf(self, filename, turn_interval=0.5, n_end_sections=2):
        ...
        # 定义排除的颜色：绿色（扫掠断面）、红色（原中心线）、品红色（投影线）
        excluded_colors = [3, 1, 6]  # 绿色、红色和品红色
...
if __name__ == "__main__":
    ...
    generator_right.save_to_dxf('波纹螺旋锥管 指数螺旋_phi_G2混接_多规扫掠.dxf',
