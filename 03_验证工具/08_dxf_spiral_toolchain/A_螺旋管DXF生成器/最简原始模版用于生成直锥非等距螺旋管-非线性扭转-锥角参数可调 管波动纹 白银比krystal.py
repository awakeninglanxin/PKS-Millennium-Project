...
class SpiralSweepGenerator:
    ...
    def _precompute_exp_wave(self):
        ...
        # 计算频率和相位
        freq = np.power(np.sqrt(2), k * t_normalized* (self.n_turns * 2 * np.pi)) / (2 * np.pi)
        ...
        if self.amp_nonlinear:
            amplitude = np.power(np.sqrt(2), t_normalized * self.n_turns)
        else:
...
if __name__ == "__main__":
    ...
    generator_right.save_to_dxf('波纹螺旋锥管 指数螺旋_sprt2.dxf',
