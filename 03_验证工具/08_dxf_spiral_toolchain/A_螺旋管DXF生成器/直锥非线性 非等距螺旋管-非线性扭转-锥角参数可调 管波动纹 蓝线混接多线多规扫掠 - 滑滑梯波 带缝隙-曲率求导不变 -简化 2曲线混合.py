...
class SpiralSweepGenerator:
    def __init__(self, start_dxf_path, end_dxf_path, bottom=300, high=300, n_turns=2.5, sign_spiral_dir=1,
                 ...
                 twist_type='linear', bands=15, use_parallel=True, phase_dif=0):
        """
        初始化螺旋扫掠生成器 - 支持并行处理
    ...
    def save_to_dxf(self, filename):
        ...
        line = msp.add_line((0, 0, z_min - 30), (0, 0, z_max + z_axis_length / 2))
        line.dxf.color = colors['axis']

        # 在z最小值位置添加同心圆
        circle_center = (0, 0, z_min- 30)

        # 添加12mm直径的圆
        circle_12mm = msp.add_circle(circle_center, 6.0)  # 半径=6mm
        circle_12mm.dxf.color = colors['circles']

        # 添加8.3mm直径的圆
        circle_8_3mm = msp.add_circle(circle_center, 4.15)  # 半径=4.15mm
        circle_8_3mm.dxf.color = colors['circles']
        ...
        print(f"在z最小值位置({z_min:.2f})添加了直径12mm和8.3mm的同心圆")
...
if __name__ == "__main__":
    ...
    for bands_value, phase_dif_value in zip(bands_list, phase_dif_list):
        ...
        generator_right = SpiralSweepGenerator(
            ...
            # 'start 舒伯格蛋咬一口曲线.dxf', 'end 圆曲线.dxf',
            bottom=160,
            high=160,
            amp=4,
            alpha_deg=13.282525,
        ...
        # 保存到DXF文件
        generator_right.save_to_dxf(f'波纹螺旋锥管_滑滑梯_多规扫掠{bands_value}段波_相位差{phase_dif_value:.2f}.dxf')
