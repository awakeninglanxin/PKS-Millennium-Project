...
def generate_plot(turns_num, a_values, n_values, filename):
    ...
    def transform_spiral(x, y):
        # 用户设定的标准高度，用于标准化投影的z轴高度
        user_high = 30  # 可以调整这个值以符合实际需要
        # 计算基于黄金比例的对数高度，用于归一化
        h1 = abs(np.log(turns_num) / np.log((-1 + np.sqrt(5)) / 2))
        # 计算新的 z_axis 坐标，考虑对数转换和高度标准化
        z_axis = (np.log(bottom_radius/ abs(x)) / np.log(phi))*user_high/h1  # 根据新的投影方程
        # 计算从原点到(x, y)点的径向距离
        r = np.sign(x) * np.sqrt(x ** 2 + y ** 2)
        return z_axis, r  # 返回标准化后的 z 坐标和径向距离
    ...
    ax2.text(0.05, 0.90, f'n1={n_values[0]}, n2={n_values[1]}, n3={n_values[2]}', transform=ax2.transAxes, fontsize=10,
             verticalalignment='top')
    ax2.text(0.05, 0.85, f'turns = {turns_num}', transform=ax2.transAxes, fontsize=10, verticalalignment='top')
