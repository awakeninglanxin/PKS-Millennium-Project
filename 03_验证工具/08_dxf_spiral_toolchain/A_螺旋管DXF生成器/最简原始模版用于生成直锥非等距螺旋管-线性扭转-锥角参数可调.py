...
class SpiralSweepGenerator:
    ...
    def update_psi(self):
        ...
        shift0to1 = 0.25
        self.psi = np.linspace(-shift0to1 * self._twist * t_length, (1 - shift0to1) * self._twist * t_length,
                               self.num_t)
...
if __name__ == "__main__":
    ...
    generator_right.save_to_dxf('线性扭转 指数螺旋_固定参数.dxf', turn_interval=0.5)  # 使用0.25圈间隔

    generator_right.sign = 1  # 管旋转180
    generator_right.r = 1
    generator_right.save_to_dxf('线性扭转 指数螺旋_反向旋转.dxf', turn_interval=0.5)  # 使用默认0.5圈间隔