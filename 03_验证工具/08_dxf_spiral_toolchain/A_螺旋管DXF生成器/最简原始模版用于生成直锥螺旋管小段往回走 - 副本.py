...
class SpiralSweepGenerator:
    def __init__(self, spline_data_path, t_min=2 * np.pi, t_max=5 * np.pi, d=4, z_v=1, amp=0.005, sign_spiral_dir=1, sign=1, twist=1):
        """
        初始化螺旋扫掠生成器
        """
        self.t_min = t_min
        ...
        self.a = np.linspace(self.amp/10, self.amp, self.num_t)
    ...
    def calculate_spiral_coords(self):
        ...
        z = np.exp(self.b * self.t_spiral) *(self.t_spiral- 3.5)
        return x, y, z
