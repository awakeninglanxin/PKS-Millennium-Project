...
class SpiralSweepGenerator:
    def __init__(self, spline_data_path=None, config_file=None, **kwargs):
        ...
        self.default_config = {
            "t_min": 0,
            "t_max": 1,
            "diameter": 25.3,#圆台锥底直径
            "pitch": 3.75/2,#锥垂直螺距的一半
            "pitch_multi": 4, #锥螺线圈匝数
            "taper_angle": 1.337,#锥顶角度除以2的值
            "sign": 1,#1顺牙攻，-1反牙攻
            "num_t": None,#总扫掠断面的数量
            "output_file": "斜螺丝锥GL25.dxf",
            "section_file": "三角牙82度.dxf",
            "triangle_size": 1,#扫掠断面大小
            "depth": -0.469,#攻牙向中轴的深度
            "twist_enabled": True,#断面扭转与锥倾斜角一致
            "dynamic_scaling": False,  # 断面缩放开关
            "show_cone_vertex": False,  # 是否显示锥形顶点
        ...
        # 其他参数
        self.pitch_multi = self.config["pitch_multi"]
    ...
    def save_to_dxf(self, filename=None):
        ...
        if center_points:
            ...
            start_z_target = self.pitch*2  # 起点高度减去一个螺距（从0开始）
            end_z_target = self.h - self.pitch*2  # 终点高度减去一个螺距
