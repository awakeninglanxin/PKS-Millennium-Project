...
class SpiralSweepGenerator:
    def __init__(self, start_dxf_path, middle_dxf_path, end_dxf_path, bottom=300, high=300, n_turns=2.5,
                 ...
                 phase_dif=0, middle_turn=1.5,k=0.2):
        """
    ...
    def sweep_section(self):
        ...
        # 使用指数变化的频率计算混合系数
        # print("in sweep k:",self.k)
        # print("in sweep middle_turn:", self.middle_turn)
    ...
    def save_to_dxf(self, filename, bands_list=None):
        ...
        # 定义排除的颜色（使用集合提高查找效率）
        excluded_colors = {3, 7, 1, 6, 4, 2}  #2是黄色，这样在生成导轨线颜色时，黄色将不会被使用，避免与Rhino的高亮颜色冲突
        ...
        for i, turn in enumerate(target_turns):
            ...
            for j in range(start_idx, end_idx):
                if j < len(swept_sections):
                    section = swept_sections[j]

                    # 根据是否为最后一个turn选择不同的点
                    if i == len(target_turns) - 1:  # 最后一个turn
                        # 找到该断面上z值最大的点
                        min_point = max(section, key=lambda p: p[2])
                    else:
                        # 找到该断面上距离z轴最近的点
                    ...
                    # 更新最小z值（仅对非最后一个turn）
                    if i != len(target_turns) - 1 and min_point[2] < find_z_min:
                        find_z_min = min_point[2]
        ...
        if bands_list is not None and self.bands == bands_list[0]:
            ...
            z_max = max(z_coords)
            z_axis_length = z_max - find_z_min
            line = msp.add_line((0, 0, find_z_min), (0, 0, z_max + z_axis_length / 2))
            ...
            # 添加outer直径的圆
            circle_outer = msp.add_circle(circle_center, outer_dia / 2)
