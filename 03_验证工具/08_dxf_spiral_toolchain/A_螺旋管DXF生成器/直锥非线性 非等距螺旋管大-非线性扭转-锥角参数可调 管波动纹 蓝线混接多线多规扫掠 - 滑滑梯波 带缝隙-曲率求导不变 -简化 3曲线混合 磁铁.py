...
class SpiralSweepGenerator:
    def __init__(self, start_dxf_path, middle_dxf_path, end_dxf_path, bottom=300, high=300, n_turns=2.5,
                 ...
                 phase_dif=0, middle_turn=1.5, k=0.2, extension_length=5.0):  # 添加 extension_length 参数
        """
        初始化螺旋扫掠生成器 - 支持三个断面和中间过渡

        参数:
        start_dxf_path: 起始断面DXF文件路径
        middle_dxf_path: 中间过渡断面DXF文件路径
        end_dxf_path: 结束断面DXF文件路径
        middle_turn: 中间断面出现的螺旋圈数位置
        """
        # 保存中间断面位置
        self.middle_turn = middle_turn
        self.extension_length = extension_length  # 保存延伸长度参数
        # 其他初始化代码保持不变...
        ...
        self.load_section_from_dxf(middle_dxf_path, section_type='middle')
    ...
    def sweep_section(self):
        ...
        # 确保三个断面点数相同
        min_points = min(len(self.start_x), len(self.middle_x), len(self.end_x))
        start_x, start_y = self.start_x[:min_points], self.start_y[:min_points]
        middle_x, middle_y = self.middle_x[:min_points], self.middle_y[:min_points]
        ...
        # 计算每个t值对应的螺旋圈数
        turns = self.n_turns * t_normalized

        # 使用指数变化的频率计算混合系数
        freq = np.power(2, self.k * t_normalized)
        # 归一化频率到[0,1]范围
        ...
        if self.use_parallel and min_points > 10:
            ...
            with Pool(processes=cpu_count()) as pool:
                ...
                for i in range(self.num_t):
                    # 根据当前螺旋圈数决定使用哪些断面进行混合
                    if turns[i] < self.middle_turn:
                        # 在中间断面出现之前：从起始断面到中间断面
                        t_blend = blend_factors[i] * (turns[i] / self.middle_turn)
                        x_blend = (1 - t_blend) * start_x + t_blend * middle_x
                        y_blend = (1 - t_blend) * start_y + t_blend * middle_y
                    else:
                        # 在中间断面出现之后：从中间断面到结束断面
                        t_blend = blend_factors[i] * ((turns[i] - self.middle_turn) / (self.n_turns - self.middle_turn))
                        x_blend = (1 - t_blend) * middle_x + t_blend * end_x
                        y_blend = (1 - t_blend) * middle_y + t_blend * end_y
        ...
        return swept_sections, center_points.tolist(), guide_lines, last_center_points  # 返回 jump_points
    ...
    def _precompute_exp_wave(self):
        ...
        def objective(k):
            ...
            return abs(phase - self.bands)  #若+0.1出水口就是窄口

        res = minimize(objective, x0=0.1, bounds=[(0.001, 1)])
        self.k = res.x[0]
        t_normalized = self.t_param

        freq = np.power(2, self.k * t_normalized * (self.n_turns * 2 * np.pi)) / (2 * np.pi)
    ...
    def save_to_dxf(self, filename, bands_list=None):
        ...
        # 定义排除的颜色（使用集合提高查找效率）
        excluded_colors = {3, 8, 9, 1, 6, 4, 2, 5}  # 举例：添加5到排除列表，避免导轨线使用蓝色
        doc = ezdxf.new(dxfversion='R2010')
        msp = doc.modelspace()
        inner_dia = 10.3  # 内直径
        thickness = 1.5  # 管壁厚
        outer_dia = inner_dia + thickness * 2
        outer_radius = outer_dia / 2  # 外圆半径
        ...
        T_unit, N_unit, B_unit = frame_vectors
        last_tangent = T_unit[-1]  # 最后一个点的切线向量

        # 将最后一个扫掠断面沿着切线方向移动指定距离
        if swept_sections and last_tangent is not None:
            last_section = swept_sections[-1]
            move_distance = self.extension_length  # 使用用户定义的延伸长度
            move_vector = last_tangent * move_distance

            # 移动最后一个断面的所有点
            moved_last_section = []
            for point in last_section:
                moved_point = np.array(point) + move_vector
                moved_last_section.append(moved_point.tolist())

            # 替换原来的最后一个断面
            swept_sections[-1] = moved_last_section

        # 将最后一个中心点也沿着切线方向移动指定距离，并添加到last_center_points列表中
        if last_center_points and last_tangent is not None:
            original_last_point = last_center_points[-1]  # 获取原始最后一个中心点
            move_distance = self.extension_length  # 使用用户定义的延伸长度
            move_vector = last_tangent * move_distance

            # 创建移动后的点
            moved_last_point = np.array(original_last_point) + move_vector
            moved_last_point = moved_last_point.tolist()
            # 将移动后的点添加到last_center_points列表中
            last_center_points.append(moved_last_point)

        # 将导轨线的每根线的最后一个点也沿着切线方向移动指定距离
        if guide_lines and last_tangent is not None:
            move_distance = self.extension_length  # 使用用户定义的延伸长度
            move_vector = last_tangent * move_distance

            for j, points in guide_lines.items():
                if points:  # 确保导轨线有至少一个点
                    # 获取最后一个点
                    last_point = points[-1]
                    # 移动最后一个点
                    moved_last_point = np.array(last_point) + move_vector
                    # 替换原来的最后一个点
                    points[-1] = moved_last_point.tolist()

        target_turns = np.arange(0, self.n_turns + 0.5, 0.5)
        find_z_min = float('inf')  # 初始化为最大值，用于寻找最小z值

        # 为每个目标位置添加投影线
        n_points = 36  # 可调参数，您可以根据需要修改这个值
        half_points = n_points // 2  # 前后各取的点数
        toward_in = 1
        for i, turn in enumerate(target_turns):
            ...
            # 获取该索引附近的一组中心点（前后各half_points个，共n_points+1个点）
            indices = []
        ...
        # 只有在bands_value等于bands_list的第一个元素时才添加中心轴和同心圆
        if bands_list is not None and self.bands == bands_list[0]:
            # 添加中心轴（黑色）
        ...
        if last_center_points:
            ...
            print(f"添加了 {len(last_center_points)} 个最后中心点到DXF文件")

        doc.saveas(filename)
        print(f"成功保存所有导轨线、扫掠断面和跳变点到 {filename}")

        # 只有在bands_value等于bands_list的第一个元素时才打印同心圆信息
        if bands_list is not None and self.bands == bands_list[0]:
            print(f"在z最小值位置({find_z_min:.2f})添加了{outer_dia:.2f}和{inner_dia:.2f}的同心圆")
if __name__ == "__main__":
    # 定义bands和对应的相位差
    bands_list = [13, 19, 15, 17]
    # bands_list = [13]
    phase_dif_list = [0, np.pi, np.pi/2, -np.pi/2]

    for bands_value, phase_dif_value in zip(bands_list, phase_dif_list):
        # 使用目标侧面夹角计算top直径并创建模型
        curve_generator = SpiralSweepGenerator(
            'start 舒伯格蛋咬一口曲线.dxf', 'end 舒伯格蛋咬一口曲线.dxf', 'end krystal 桃子.dxf',
            bottom=140,
            high=140*silver_ratio,
            amp=3,
            alpha_deg=6.7315,
            dxf_points=19,
            twist_type='exponential',
            use_parallel=False,
            bands=bands_value,
            phase_dif=phase_dif_value,  # 添加相位差参数
            extension_length=5.0  # 可以设置为任何您想要的延伸长度
        )
        ...
        # 保存到DXF文件
        curve_generator.save_to_dxf(f'波纹螺旋锥管_滑滑梯_多规扫掠{bands_value}段波_相位差{phase_dif_value:.2f}.dxf', bands_list)

        print(f"已生成 {bands_value} 段波，相位差 {phase_dif_value:.2f} 的文件")