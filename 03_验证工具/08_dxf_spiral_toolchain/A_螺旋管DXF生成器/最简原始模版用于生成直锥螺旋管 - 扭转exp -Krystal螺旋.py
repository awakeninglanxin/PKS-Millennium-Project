...
class SpiralSweepGenerator:
    ...
    def calculate_spiral_coords(self):
        """计算螺旋线的坐标"""
        x = 4 **(self.t_spiral/(np.pi)) * np.sin(self.t_spiral) * self.r
        y = 4 **(self.t_spiral/(np.pi)) * np.cos(self.t_spiral)
        z = 4 **((self.t_spiral)/(np.pi))*self.z_v
        return x, y, z
...
if __name__ == "__main__":
    ...
    generator_right.save_to_dxf('最简优先 对扭转—口朝内(头朝下) krystal.dxf')
    generator_right.sign = 1  # 管旋转180
    generator_right.r = 1

    generator_right.save_to_dxf('螺旋锥 对扭转—口朝内(头朝下) krystal.dxf')