...
def plot_egg_curves(n_max=5, amp_scheme="inverse", branch_to_show="both"):
    """绘制不同参数组的蛋形曲线旋转叠加效果"""
    fig, ax = plt.subplots(1, 1, figsize=(10, 10))

    # 设置颜色映射 - 蛋外曲线使用彩虹色，branch2使用HSV色
    ...
    for n in range(1, n_max + 1):
        # 使用斐波那契数列生成k和b
        fib_n = fibonacci(n + 1)  # Fₙ
        fib_n1 = fibonacci(n + 2)  # Fₙ₊₁
        fib_n2 = fibonacci(n + 3)  # Fₙ₊₂
        ...
        # 根据用户选择的显示分支
        if amp_scheme == "max_n" and n != n_max:  # 只显示最大n值的曲线
            continue
...
def main():
    ...
    # 让用户选择显示的曲线分支
    branch_choice = input("请选择要显示的分支 (1: 只显示蛋外曲线, 2: 只显示蛋曲线, 3: 显示蛋外曲线和蛋曲线, 默认3): ") or "3"
    if branch_choice == "1":
        branch_to_show = "蛋外曲线"
    ...
    plot_egg_curves(n_max, amp_scheme, branch_to_show)
