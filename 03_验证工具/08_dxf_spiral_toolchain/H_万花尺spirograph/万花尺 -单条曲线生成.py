...
class ArbitrarySpirograph:
    ...
    def calculate_perfect_rotations(self):
        ...
        # 计算需要的最小圈数 N，使得 N × p × a 能被 q × b 整除
        # 这等价于 N 是 (q × b) / gcd(p × a, q × b) 的倍数
...
class SpirographApp:
    ...
    def display_curve(self):
        ...
        if self.current_curve_index < len(self.dxf_reader.curves):
            ...
            self.ax.clear()
            self.ax.plot(curve_points[:, 0], curve_points[:, 1], 'r-', linewidth=2, label='原始曲线')
            self.ax.set_aspect('equal')
            self.ax.grid(True, linestyle='--', alpha=0.7)
            self.ax.legend()

            min_x, min_y, max_x, max_y = self.dxf_reader.get_curve_bounds(self.current_curve_index)
    ...
    def auto_set_perfect_rotations(self):
        """自动设置完美圈数"""
        if self.spirograph.curve_points is None:
        ...
        try:
            ...
            # 显示计算详情
            info_msg = (f"完美圈数计算详情:\n\n"
                        f"齿轮比: {gear_num}/{gear_den}\n"
                        f"笔尖位置: {pen_num}/{pen_den}\n"
                        f"计算出的完美圈数: {perfect_rotations}\n\n"
                        f"数学原理:\n"
                        f"需要满足: 旋转圈数 × (齿轮比) × (笔尖位置) 为整数")

            messagebox.showinfo("完美圈数计算", info_msg)
    ...
    def check_curvature(self):
        ...
        try:
            ...
            self.spirograph.set_parameters(gear_num, gear_den,
                                           self.pen_num_var.get(),
                                           self.pen_den_var.get(),
                                           direction)
    ...
    def clear_pattern(self):
        """清除图案"""
        if self.spirograph.curve_points is not None:
            self.display_curve()
