...
# 生成弹簧的数据
def generate_spring_data(turns, points_per_turn, heights, twist_factor):
    t = np.linspace(0, turns * 2 * np.pi, int(turns * points_per_turn))
    r = a / (t + 1)  # 半径随着t线性变化

    # 转换为笛卡尔坐标系并加入扭转
    x = r * np.cos(t + twist_factor * t)
    y = r * np.sin(t + twist_factor * t)
...
# 生成斐波那契数列弹簧的数据
fib_x, fib_y, fib_z, fib_d = generate_spring_data(turns, points_per_turn, fib_heights, k)

# 生成对数增长高度弹簧的数据
log_x, log_y, log_z, log_d = generate_spring_data(turns, points_per_turn, log_heights, k)
