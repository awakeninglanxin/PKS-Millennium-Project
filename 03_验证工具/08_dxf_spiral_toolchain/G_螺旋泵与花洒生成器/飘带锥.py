...
for i in range(num_t):
    # 计算 ̂T(t) 切线向量
    T_unit[i, 0] = np.cos(t[i]) / np.sqrt(1 + z_v ** 2)
    T_unit[i, 1] = np.sin(t[i]) / np.sqrt(1 + z_v ** 2)
    T_unit[i, 2] = -z_v / np.sqrt(1 + z_v ** 2)

    # 计算 ̂N(t) 法向量
    N_unit[i, 0] = -np.sin(t[i])
    N_unit[i, 1] = np.cos(t[i])
    N_unit[i, 2] = 0

    # 计算 ̂B(t) 副法向量
    B_unit[i, 0] = z_v * np.cos(t[i])
    B_unit[i, 1] = z_v * np.sin(t[i])
    B_unit[i, 2] = 1
