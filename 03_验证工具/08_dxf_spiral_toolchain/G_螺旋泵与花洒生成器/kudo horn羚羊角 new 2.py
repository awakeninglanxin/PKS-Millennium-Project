...
phi = np.linspace(0, -np.pi/2, num_t)  # 避免 t = 0 的问题
...
psi =np.linspace(-t_length /2,t_length /2 , num_t)  # 避免 t = 0 的问题
...
for i in range(num_t):
    for j in range(num_theta):
        ...
        # 根据公式生成椭圆在局部坐标系中的分量
        N_term = (x(theta[j], 5 / 3, 2 / 3, 1) * np.cos(phi[i]) - y(theta[j], 5 / 3, 2 / 3, 1, 2 / 3) * np.sin(phi[i])) * N_unit[i]
        B_term = (x(theta[j], 5 / 3, 2 / 3, 1) * np.sin(phi[i]) + y(theta[j], 5 / 3, 2 / 3, 1, 2 / 3) * np.cos(phi[i])) * B_unit[i]
