...
class SpiralSweepGenerator:
    ...
    def calculate_spiral_coords(self):
        ...
        t = self.t_spiral
        x_spiral = np.sin(3 * self.f * t) * (np.sqrt(1 / (self.b - t * np.sin(self.angle)) ** 2 - (t * np.cos(self.angle)) ** 2))
        y_spiral = np.cos(3 * self.f * t) * (np.sqrt(1 / (self.b - t * np.sin(self.angle)) ** 2 - (t * np.cos(self.angle)) ** 2))
        z_spiral = -t * self.z_v
