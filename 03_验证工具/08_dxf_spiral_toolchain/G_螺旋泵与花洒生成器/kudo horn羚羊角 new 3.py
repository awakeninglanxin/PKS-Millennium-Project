...
# 生成mesh圆盘的函数
def generate_circle_mesh_with_faces(radius, z_value, num_points=100):
    # 生成圆周上的角度
    theta = np.linspace(0, 2 * np.pi, num_points)
    # 计算 x 和 y 坐标
    x = radius * np.cos(theta)
    y = radius * np.sin(theta)
    # 创建一个 z 坐标数组，所有 z 坐标都等于给定的 z_value
    z = np.full_like(x, z_value)

    # 将圆心作为第一个顶点添加
    vertices = np.vstack((np.zeros((1, 3)), np.column_stack((x, y, z))))

    # 生成面数组，连接中心点和圆周上的点
    faces = []
    for i in range(1, num_points):
        faces.append([0, i, i + 1])  # 连接中心点和相邻的两个圆周点
    faces.append([0, num_points, 1])  # 处理最后一个面，连接回起始点
...
for i in range(num_t):
    for j in range(num_theta):
        ...
        R_z = rotation_matrix_z(theta_z[i])
        # 椭圆点，并应用尺度变化因子和旋转矩阵
        vector=np.dot(R_theta, (N_term + B_term))
        ellipse_point = scale_factor * np.dot(R_z, vector)
...
max_index = np.sqrt(xy_square_sum[max_index])
# 打印最小值的平方根
...
for z in z_values:
    # 找到接近当前 z 值的所有点
    close_points_mask = np.isclose(vertices_combined[:, 2], z, atol=1e-5)
...
for disc in discs:
    z_value, radius = disc
    # 为每个圆盘生成顶点和面
    vertices, faces = generate_circle_mesh_with_faces(radius, z_value)
