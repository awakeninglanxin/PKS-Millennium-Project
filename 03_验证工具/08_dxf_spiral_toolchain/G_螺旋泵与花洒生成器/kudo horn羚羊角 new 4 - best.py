...
for i in range(num_t):
    # 计算 \hat{T}(t) 切线向量
    T_unit[i, 0] = d * (b * (np.sin(t[i]) + np.cos(t[i]))) / np.sqrt((b ** 2 + 1) * (d ** 2) + (b ** 2) * (z_v ** 2))
    T_unit[i, 1] = d * (b * (np.cos(t[i]) - np.sin(t[i]))) / np.sqrt((b ** 2 + 1) * (d ** 2) + (b ** 2) * (z_v ** 2))
    T_unit[i, 2] = -b * z_v / np.sqrt((b ** 2 + 1) * (d ** 2) + (b ** 2) * (z_v ** 2))
    ...
    # 计算 \hat{B}(t) 副法向量
    B_unit[i, 0] = b * z_v* (b * np.sin(t[i]) + np.cos(t[i])) / np.sqrt((b ** 2 + 1) * (d ** 2) + (b ** 2) * (z_v ** 2))
    B_unit[i, 1] = b * z_v* (b * np.cos(t[i]) - np.sin(t[i])) / np.sqrt((b ** 2 + 1) * (d ** 2) + (b ** 2) * (z_v ** 2))
    B_unit[i, 2] = d * (b ** 2 + 1) / np.sqrt((b ** 2 + 1) * (d ** 2) + (b ** 2) * (z_v ** 2))
...
for i in range(num_t):
    for j in range(num_theta):
        ...
        # 根据公式生成椭圆在局部坐标系中的分量
        N_term = (x(theta[j], a[i]) * np.cos(phi[i]) - y(theta[j], a[i]) * np.sin(phi[i])) * N_unit[i]
        B_term = (x(theta[j],  a[i]) * np.sin(phi[i]) + y(theta[j],  a[i])* np.cos(phi[i])) * B_unit[i]
        T_term = z(theta[j],  a[i]) * T_unit[i]
...
mesh.export('kudo_horn_with_axis_array.obj')
