...
class SpiralSweepGenerator:
    ...
    def __init__(self, t_min=0, t_max=2*np.pi/6, d=4.31, z_v=0.3487125*1.08, amp=0.25,
    ...
    def calculate_spiral_coords(self):
        x = (-np.cos(self.t_spiral)-np.cos(7*self.t_spiral)/3)*np.exp(np.log(2)*self.t_spiral/(np.pi))* self.r
        y = (np.sin(self.t_spiral)+np.sin(7*self.t_spiral)/3)*np.exp(np.log(2)*self.t_spiral/(np.pi))
        z = self.t_spiral
        return x, y, z
    ...
    def sweep_section(self):
        ...
        # 生成 t 从 π 到 3π 的值
        t_values = np.linspace(np.pi, 2.5 * np.pi, 16)
...
def main():
    ...
    generator.r = 1
    generator.twist = 1
    generator.save_to_dxf('螺旋摆线向内_1.dxf')

    # 第二个模型
    generator.sign = -1
    generator.r = 1
    generator.set_default_psi()
    generator.save_to_dxf('螺旋摆线向内_2.dxf')

    # 第三个模型
    generator.sign = 1
    generator.r = 1
    generator.set_custom_psi(-np.pi / 2, generator.t_length / 2 - np.pi / 2)
    generator.save_to_dxf('螺旋摆线向内_3.dxf')
