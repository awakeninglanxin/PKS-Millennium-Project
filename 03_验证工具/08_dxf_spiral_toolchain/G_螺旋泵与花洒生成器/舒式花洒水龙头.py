...
class SpiralSweepGenerator:
    def __init__(self, spline_data_path, b=1.6, t_min=0.45*np.pi, t_max=0.7*np.pi, d=1.5, z_v=1, amp=0.03, sign_spiral_dir=1, sign=1, twist=1,angle=0.588):
        """
        初始化螺旋扫掠生成器
        """
        self.t_min = t_min
        ...
        self.y = np.linspace(1, 1/2, self.num_t)
    ...
    def calculate_spiral_coords(self):
        ...
        t = self.t_spiral
        x_spiral = np.sin(3 * self.d * t) * (np.sqrt(1 / (self.b - t * np.sin(self.angle)) ** 2 - (t * np.cos(self.angle)) ** 2))
        y_spiral = np.cos(3 * self.d * t) * (np.sqrt(1 / (self.b - t * np.sin(self.angle)) ** 2 - (t * np.cos(self.angle)) ** 2))
        z_spiral = -t * self.z_v
    ...
    def sweep_section(self):
        ...
        for i in range(self.num_t):
            ...
            for j in range(len(self.section_x)):
                ...
                # 计算 ellipse2 的每个点的坐标
                B_term1 = self.sign * x(j) * B_unit[i]*self.y[i]
                N_term1 = y(j) * N_unit[i]
