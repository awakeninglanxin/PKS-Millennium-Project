...
def create_cylinders_at_points(projected_points, radius, height):
    ...
    for point, direction in projected_points:
        # Create a plane at the projected point for the thin cylinder (small radius)
        plane = rs.PlaneFromNormal(point, direction)  # Define a plane at the point with ray direction as normal
        
        # Create a base circle (this base_circle will now represent the plane, not just a simple circle)
        base_circle =rs.AddCircle(plane, radius)
        base_surface = rs.AddPlanarSrf(base_circle) 
        # Create a larger base circle for the thick cylinder (larger radius)
        thick_base_circle = rs.AddCircle(plane, radius + 0.5)  # Increase the radius by 0.5mm
        ...
        # Perform the sweep to create the thick cylinder (large radius)
...
if surface_id:
    ...
    if surface:
        ...
        num_points = 33  # Number of points to generate
