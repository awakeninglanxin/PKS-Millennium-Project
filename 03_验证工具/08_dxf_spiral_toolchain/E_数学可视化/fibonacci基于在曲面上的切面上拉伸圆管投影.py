...
def create_cylinders_at_points(projected_points, radius, height):
    ...
    for point, direction in projected_points:
        ...
        path_end = point + normal * height  # Extend outward along the normal
        path = rs.AddLine(path_start, path_end)
        
        # Create the base circle (this will now represent the plane, not just a simple circle)
        base_circle = create_base_circle_at_point(point, radius)  # Create the base circle using the cross logic
        
        if base_circle is None:
            continue  # Skip this point if the base circle couldn't be created
        
        # Create the thicker base circle for the larger tube
        thick_base_circle = create_base_circle_at_point(point, radius + 0.5)  # Increase radius for thick tube
        
        if thick_base_circle is None:
            continue  # Skip this point if the thick base circle couldn't be created
        
        # Perform the sweep to create the thin cylinder (small radius)
        thin_cylinder = rs.AddSweep1(path, [base_circle], closed=True)
        if thin_cylinder:
            # Cap the thin cylinder to close the ends
            rs.CapPlanarHoles(thin_cylinder)
        
        # Perform the sweep to create the thick cylinder (larger radius)
        thick_cylinder = rs.AddSweep1(path, [thick_base_circle], closed=True)
        if thick_cylinder:
            # Cap the thick cylinder to close the ends
            rs.CapPlanarHoles(thick_cylinder)
        
        # Use the Boolean Difference operation to subtract the thin cylinder from the thick cylinder
        if thin_cylinder and thick_cylinder:
            result = rs.BooleanDifference(thick_cylinder, thin_cylinder)
            if result:
                cylinders.append(result[0])  # Add the resulting tube (hollow cylinder)
        
        # Clean up temporary geometry
        rs.DeleteObject(base_circle)
        rs.DeleteObject(thick_base_circle)
        rs.DeleteObject(path)
        if thin_cylinder:
            rs.DeleteObject(thin_cylinder)
        if thick_cylinder:
            rs.DeleteObject(thick_cylinder)
...
if surface_id:
    ...
    if surface:
        ...
        cylinder_radius = 5.55  # Radius of the cylinder
