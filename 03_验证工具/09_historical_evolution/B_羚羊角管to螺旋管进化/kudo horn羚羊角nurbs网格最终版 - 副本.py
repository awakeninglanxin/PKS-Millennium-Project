...
class NurbsSurfaceGenerator:
    ...
    def _interpolate_coordinate(self, coord_data, u, v):
        ...
        try:
            ...
            return coord_data  # 如果插值失败，返回原始数据
    ...
    def generate_nurbs_surface(self, points):
        ...
        Parameters:
        points: 3D array (m x n x 3), 控制点网格
        ...
        try:
            ...
            return points  # 如果失败则返回原始点云

    def _validate_inputs(self, points):
        """验证输入数据的有效性"""
        if not isinstance(points, np.ndarray):
            raise ValueError("输入点必须是numpy数组")

        if len(points.shape) != 3:
            raise ValueError("输入点必须是3维数组 (m x n x 3)")

        if points.shape[2] != 3:
            raise ValueError("每个点必须有3个坐标分量 (x, y, z)")
...
class MeshBuilder:
    ...
    def add_center_cylinder(self, min_z, max_z):
        ...
        z = np.linspace(min_z, max_z * 1, self.params.num_t)
    ...
    def calculate_surface(self):
        ...
        for i in range(self.params.num_t):
            for j in range(self.params.num_theta):
                # 处理theta值超出范围的情况
                theta_val = self.theta[j] % (2*np.pi)
                x_val = self.x_interp(theta_val) * self.a[i]
                y_val = self.y_interp(theta_val) * self.a[i]

                B_term = (x_val * np.cos(self.phi[i]) -
                          y_val * np.sin(self.phi[i])) * self.B_unit[i]
                N_term = (x_val * np.sin(self.phi[i]) +
                          y_val * np.cos(self.phi[i])) * self.N_unit[i]
