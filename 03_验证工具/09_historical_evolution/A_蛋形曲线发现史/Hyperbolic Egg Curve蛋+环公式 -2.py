...
# Functions
def hyperbolic_egg_curve(k, b, num_points=num_p):
    ...
    return a * (2 * np.sin(t) / (z0 + np.sqrt(b ** 2 - 4 * k * np.cos(t)))),a * (m * term1 + term2)


# Initial Parameters
alpha_deg_k = 2/3
alpha_val = alpha_deg_k * np.pi / 180  # Example value for alpha (in radians)
z0 = 5 / 3  # Example value for z0
a1, b1 = hyperbolic_egg_curve(alpha_deg_k, z0, num_points=num_p)
torus = []
t = np.linspace(0, 2 * np.pi, num_p)

# 修改后的 parametric_curve_3d 函数
def parametric_curve_3d(a, b, alpha_deg_k, z0, c, d):
    global ax_3d
    ax_3d.clear()  # 清除之前的绘图
...
def plot_hyperbolic_egg_curve(alpha_deg_k, z0):
    x_minus, y_minus = hyperbolic_egg_curve(alpha_deg_k, z0, num_points=num_p)
    ax_2d.clear()
...
d = 3
plot_hyperbolic_egg_curve(alpha_deg_k, z0)
parametric_curve_3d(a, b, alpha_val, z0, c, d)

# Slider setup
ax_alpha_deg_k = plt.axes([0.1, 0.25, 0.65, 0.03])
...
slider_alpha_deg_k = Slider(ax_alpha_deg_k, 'alpha_deg_k', 0, 3, valinit=alpha_deg_k)
slider_z0 = Slider(ax_z0, 'z0', 0.000001, 4, valinit=z0)
slider_c = Slider(ax_c, 'c', 1, 30, valinit=c, valstep=1)

def update(val):
    # 获取滑动条的值
    alpha_deg_k = slider_alpha_deg_k.val  # 保持为角度制
    a = slider_a.val
    b = slider_b.val
    ...
    # 更新2D图
    plot_hyperbolic_egg_curve(alpha_deg_k, z0_val)
    # 更新3D图
    parametric_curve_3d(a, b, alpha_deg_k, z0_val, c_val, d)

slider_alpha_deg_k.on_changed(update)
