...
def x_minus(t, b, k, a, t_min):
    return a * ((2 * np.sin(t) / (b + np.sqrt(b**2 - 4 * k * np.cos(t)))) - t_min/2)
...
def y_add(t, b, k, a, m, t_min):
    ...
    term2 = ((1 / (2*np.sqrt(1 + k**2))) *
            ...
            ((b * (-1 + k**2) + np.sqrt(b**2 - 4*k) * (1 + k**2)) /
             (2*k*np.sqrt(1 + k**2)))
    return a * ((m*term1 + term2) + t_min/2)

# 使得k、b随时间t变化的函数

def b_fun(t, user_num):
    T=2*np.pi
    return 2*t /(T*np.sqrt(t))# 化简后的公式

def k_fun(t, user_num):
    T = 2 * np.pi
    return 1/T  # 化简后的公式


# 计算曲线（不使用amp衰减因子），k、b随时间t变化
x_vals = x_fun(t, b_fun(t, user_num), k_fun(t, user_num), a)
y_vals = y_fun(t, b_fun(t, user_num), k_fun(t, user_num), a, m)
x_vals_add = x_minus(t, b_fun(t, user_num), k_fun(t, user_num), a, t_min)
y_vals_minus = y_add(t, b_fun(t, user_num), k_fun(t, user_num), a, m, t_min)
