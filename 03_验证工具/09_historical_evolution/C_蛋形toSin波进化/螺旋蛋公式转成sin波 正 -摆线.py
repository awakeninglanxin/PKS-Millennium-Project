...
def x(t, b, k, a, d, c, f):
    return -a * ((np.cos(d * t - c) * 2 * np.sin(f * t)) / (b + np.sqrt(b ** 2 - 4 * k * np.cos(f * t))))
def x_add(t, b, k, a, d, c, f):
    return -a * ((np.cos(d * t - c) * 2 * np.sin(f * t)) / (b + np.sqrt(b ** 2 - 4 * k * np.cos(f * t)))+1/t)
...
def y_minus(t, b, k, a, m, d, c, f):
    ...
    term3 = -((b * (-1 + k ** 2) + np.sqrt(b ** 2 - 4 * k) * (1 + k ** 2)) / (2 * k * np.sqrt(1 + k ** 2)))
    return -a * ((np.sin(d * t + c)) * m * (term1 + term2 + term3)-1/t)
