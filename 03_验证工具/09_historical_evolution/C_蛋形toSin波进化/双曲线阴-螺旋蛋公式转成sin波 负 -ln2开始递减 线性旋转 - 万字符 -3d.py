...
def amp_continuous(t):
    ...
    denominator = n * (n + 1)
    # return 1  # 螺旋微微漏斗
    # return 1/np.sqrt(numerator / denominator) #螺旋直水滴管
    return 1/(t*np.sqrt(numerator / denominator)) #螺旋双曲水滴漏斗管
...
def b_fun(t):
    return 2*np.sqrt(((t/T))/((t/T)+1)) +0.000000000000001
