...
def x_add(t, b, k, a):
    return -a * ((2 * np.sin(t) / (b + np.sqrt(b ** 2 - 4 * k * np.cos(t))))/t+a_1*(-np.log(2*np.pi)/np.log(0.618)+np.log(t)/np.log(0.618)))
...
def y_minus(t, b, k, a, m):
    ...
    term2 = ((1 / (2 * np.sqrt(1 + k ** 2))) * (
            ((k ** 2 - 1) / k * b) + ((k ** 2 + 1) / k * np.sqrt(b ** 2 - 4 * k * np.cos(t))))) - (
                ((b * (-1 + k ** 2) + np.sqrt(b ** 2 - 4 * k) * (1 + k ** 2)) / (2 * k * np.sqrt(1 + k ** 2))))
    return -a * ((m * term1 + term2)/t+a_1*(-np.log(2*np.pi)/np.log(0.618)+np.log(t)/np.log(0.618)))
...
plt.figure(figsize=(10, 6))
plt.plot(x_vals_add, y_vals_minus, label='Spiral Curve')
plt.xlabel('x(t)')
plt.ylabel('y(t)')
