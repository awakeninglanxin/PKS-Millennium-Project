...
# Update layout
fig.update_layout(
    title=dict(text=f"total wealth: {sum(results_E)}<br>Gini Coefficient: {gini_coefficient:.3f}<br>Poorest Salary: {poorest_salary}<br>Richest Salary: {richest_salary}<br>Population: {sampleCount}", x=0.5, xanchor='center', font=dict(size=14)),
