...
a = np.linspace(0.1, 0.1, num_t)
# 计算螺旋曲线 γ(t) 包含参数 d 进行水平偏移
x_spiral = -np.sin(3*f*t)*(np.sqrt(1/(b-t*np.sin(angle))**2-(t*np.cos(angle))**2))  # 使用 exp(bt) 增加膨胀效果
y_spiral = np.cos(3*f*t)*(np.sqrt(1/(b-t*np.sin(angle))**2-(t*np.cos(angle))**2))
z_spiral =-t* z_v# 修正 z 轴分量为线性关系
...
# psi = np.linspace(0, 0, num_t)  # 负扭转
psi = np.linspace(0,-3*t_length/2, num_t)  #正扭转
...
mesh.export('generator_with_axis_array.obj')
