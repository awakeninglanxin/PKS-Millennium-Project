...
class SpiralSweepGenerator:
    def __init__(self, spline_data_path, t_min=2 * np.pi, t_max=7 * np.pi, d=15, z_v=2*(170/280)/9.6925, amp=1.5, sign_spiral_dir=1, sign=1, twist=1):
        """
        初始化螺旋扫掠生成器

        参数:
        spline_data_path: 字符串，CSV文件的路径，包含断面数据
        t_min: 浮点数，螺旋线参数最小值
        t_max: 浮点数，螺旋线参数最大值
        d: 浮点数，螺旋线直径系数
        z_v: 浮点数，z方向速度系数
        amp: 浮点数，振幅
        sign_spiral_dir: 整数，控制螺旋线的旋转方向（1 或 -1）
        sign: 整数，控制螺旋方向
        """
        self.t_min = t_min
        self.t_max = t_max
    ...
    def calculate_spiral_coords(self):
        ...
        z = self.t_spiral ** np.e * self.z_v - self.t_min ** np.e * self.z_v
        return x, y, z
...
if __name__ == "__main__":
    ...
    generator_right.save_to_dxf('t_e最简优先 对扭转—口朝内(头朝下).dxf')
    generator_right.sign = 1   # 管旋转180
    generator_right.r = 1

    generator_right.save_to_dxf('t_e 螺旋锥 对扭转—口朝内(头朝下).dxf')