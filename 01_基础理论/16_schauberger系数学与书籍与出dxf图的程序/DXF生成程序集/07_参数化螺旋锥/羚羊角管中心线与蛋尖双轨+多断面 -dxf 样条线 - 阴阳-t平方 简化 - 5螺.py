...
class SpiralSweepGenerator:
    ...
    def __init__(self, spline_data_path, t_min=3*np.pi, t_max=8 * np.pi, d=5, z_v=1, amp=1,
        ...
        self.a = np.linspace(self.amp, self.amp / 1.5, self.num_t)
...
def main():
    ...
    generator_right.save_to_dxf('t平方最简优先 对扭转—口朝内(头朝下).dxf')

    # 第二个模型
    generator_right.sign = 1
    generator_right.r = 1
    generator_right.set_default_psi()  # 使用默认的psi值
    generator_right.save_to_dxf('t平方 螺旋锥 对扭转—口朝内(头朝外).dxf')

    # 新增的第三个模型，使用修改后的psi值
    generator_right.sign = 1
    generator_right.r = 1
    generator_right.set_custom_psi(-np.pi / 2, generator_right.t_length / 2 - np.pi / 2)  # 设置新的psi值
    generator_right.save_to_dxf('t平方 螺旋锥 对扭转—口朝内(头朝下).dxf')
