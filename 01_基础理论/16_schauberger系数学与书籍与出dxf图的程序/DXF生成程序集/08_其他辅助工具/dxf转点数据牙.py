...
def extract_curve_from_dxf(dxf_path, num_points=34):
    """
    从DXF文件中提取曲线数据（包括直线和圆弧），并将参数范围映射到0-2π。
    ...
    try:
        ...
        for entity in msp:
            if entity.dxftype() == 'LINE':
                ...
                for t in t_local:
                    x = (1 - t) * start_point.x + t * end_point.x
                    y = (1 - t) * start_point.y + t * end_point.y
                    all_points.append((t * 2 * np.pi, x, y))  # 将t映射到0到2π

        # 处理ARC类型
        for entity in msp:
            if entity.dxftype() == 'ARC':
                arc_count += 1
                print(f"处理第 {arc_count} 条圆弧")
                ...
                # 在圆弧上均匀采样
                t_local = np.linspace(start_angle, end_angle, num_points)
                for angle in t_local:
                    x = center.x + radius * np.cos(angle)
                    y = center.y + radius * np.sin(angle)
                    t = (angle - start_angle) / (end_angle - start_angle) * 2 * np.pi
                    all_points.append((t, x, y))  # 将t映射到0到2π

        if line_count == 0 and arc_count == 0:
            print("警告：未在DXF文件中找到直线或圆弧类型的曲线")
            return None
        ...
        print(f"\n处理完成:")
        print(f"共发现 {line_count} 条直线")
        print(f"共发现 {arc_count} 条圆弧")
...
if __name__ == "__main__":
    ...
    parser.add_argument('dxf_path', nargs='?', default="三角牙60度.dxf",
                        help='DXF文件路径')
    parser.add_argument('--output', '-o', default="三角牙60度.csv",
                        help='输出CSV文件路径')
    parser.add_argument('--points', '-n', type=int, default=36,
                        help='采样点数量')
    ...
    points = extract_curve_from_dxf(args.dxf_path, args.points)
