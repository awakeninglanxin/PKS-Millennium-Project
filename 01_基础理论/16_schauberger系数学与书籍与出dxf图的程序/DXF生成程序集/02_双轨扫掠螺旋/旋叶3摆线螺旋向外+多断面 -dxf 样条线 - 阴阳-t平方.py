...
class SpiralSweepGenerator:
    ...
    def calculate_spiral_coords(self):
        x = -(np.cos(self.t_spiral)-np.cos(5*self.t_spiral)/3)*np.exp(np.log(2)*self.t_spiral/(np.pi))* self.r
        y =(np.sin(self.t_spiral)+np.sin(5*self.t_spiral)/3)*np.exp(np.log(2)*self.t_spiral/(np.pi))
        z = self.t_spiral
        return x, y, z
...
def main():
    ...
    generator.r = 1
    generator.twist = 1
    generator.save_to_dxf('螺旋摆线向外_1.dxf')

    # 第二个模型
    generator.sign = -1
    generator.r = 1
    generator.set_default_psi()
    generator.save_to_dxf('螺旋摆线向外_2.dxf')

    # 第三个模型
    generator.sign = 1
    generator.r = 1
    generator.set_custom_psi(-np.pi / 2, generator.t_length / 2 - np.pi / 2)
    generator.save_to_dxf('螺旋摆线向外_3.dxf')
