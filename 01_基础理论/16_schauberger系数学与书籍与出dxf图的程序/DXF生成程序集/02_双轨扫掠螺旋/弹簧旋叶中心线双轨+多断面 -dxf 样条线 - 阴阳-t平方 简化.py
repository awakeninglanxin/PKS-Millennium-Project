...
class SpiralSweepGenerator:
    ...
    def __init__(self, spline_data_path, t_min=0, t_max=8 * np.pi, d=3, z_v=5, amp=3,
    ...
    def sweep_section(self):
        ...
        swept_sections = []
        egghead_points = []
        t_values = np.linspace(np.pi, 2 * np.pi, 16)
        for i in range(self.num_t):
            # 先进行缩放
...
def main():
    ...
    generator_right.save_to_dxf('t平方最简优先 弹簧旋叶 对扭转—口朝内(头朝下).dxf')

    # 第二个模型
    generator_right.sign = 1
    generator_right.r = 1
    generator_right.set_default_psi()  # 使用默认的psi值
    generator_right.save_to_dxf('t平方 弹簧旋叶 对扭转—口朝内(头朝外).dxf')
    ...
    generator_right.save_to_dxf('t平方 弹簧旋叶 对扭转—口朝内(头朝内).dxf')
