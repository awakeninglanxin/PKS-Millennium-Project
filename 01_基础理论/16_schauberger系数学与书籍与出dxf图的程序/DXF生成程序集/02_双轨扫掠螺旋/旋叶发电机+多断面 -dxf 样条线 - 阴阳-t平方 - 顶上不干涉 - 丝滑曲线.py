...
class SpiralSweepGenerator:
    ...
    def __init__(self, t_min=0, t_max=2 * np.pi / 3, d=4.31, z_v=2, amp=0.6,
    ...
    def calculate_spiral_coords(self):
        ...
        t = self.t_spiral
        x_spiral = np.sin(3 * self.f * t) * (
            np.sqrt(1 / (self.b - t * np.sin(self.angle)) ** 2 - (t * np.cos(self.angle)) ** 2))
        y_spiral = np.cos(3 * self.f * t) * (
            np.sqrt(1 / (self.b - t * np.sin(self.angle)) ** 2 - (t * np.cos(self.angle)) ** 2))
        z_spiral = -t * self.z_v
    ...
    def generate_spiral_curves(self):
        ...
        # 使用参数方程的方式重建曲线
        # 生成参数 t (0到1之间的归一化参数)
        t = np.linspace(0, 1, self.num_t)

        # 对 x, y, z 分别进行N阶多项式拟合
        N=7
        x_coeffs = np.polyfit(t, x_spiral, N)
        y_coeffs = np.polyfit(t, y_spiral, N)
        z_coeffs = np.polyfit(t, z_spiral, N)

        # 使用拟合的多项式重建曲线点
        x_spiral = np.polyval(x_coeffs, t)
        y_spiral = np.polyval(y_coeffs, t)
        z_spiral = np.polyval(z_coeffs, t)

        # 计算一阶导数（用于切向量）
        dx_coeffs = np.polyder(x_coeffs)
        dy_coeffs = np.polyder(y_coeffs)
        dz_coeffs = np.polyder(z_coeffs)

        dx = np.polyval(dx_coeffs, t)
        dy = np.polyval(dy_coeffs, t)
        dz = np.polyval(dz_coeffs, t)

        # 计算二阶导数（用于法向量）
        ddx_coeffs = np.polyder(dx_coeffs)
        ddy_coeffs = np.polyder(dy_coeffs)
        ddz_coeffs = np.polyder(dz_coeffs)

        ddx = np.polyval(ddx_coeffs, t)
        ddy = np.polyval(ddy_coeffs, t)
        ddz = np.polyval(ddz_coeffs, t)
...
def main():
    ...
    generator.r = -1
    generator.twist = 1
    generator.save_to_dxf('丝滑旋叶发电spiral_model_1.dxf')
    ...
    generator.save_to_dxf('丝滑旋叶发电spiral_model_2.dxf')
    ...
    generator.save_to_dxf('丝滑旋叶发电spiral_model_3.dxf')
