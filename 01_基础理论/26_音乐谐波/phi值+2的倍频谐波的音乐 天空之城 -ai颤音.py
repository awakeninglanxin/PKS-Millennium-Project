    ...
    for amplitude, harmonic_freq, harmonic_num, harmonic_type in (octave_harmonics + fibonacci_harmonics):
        if 20 <= harmonic_freq <= 20000:
            ...
            if harmonic_type == "octave":
                ...
                # 斐波那契谐波：添加轻微颤音和相位调制
                # 轻微颤音（频率调制）
                vibrato_depth = 0.002 * (abs(harmonic_num) + 1)
                vibrato_rate = 5 + harmonic_num * 0.5
                freq_mod = harmonic_freq * (1 + vibrato_depth * np.sin(2 * np.pi * vibrato_rate * t))

                # 相位调制增加丰富度
                phase_mod = 0.05 * np.sin(2 * np.pi * 3 * t)
                wave = np.sin(2 * np.pi * freq_mod * t + phase_offset + phase_mod)
