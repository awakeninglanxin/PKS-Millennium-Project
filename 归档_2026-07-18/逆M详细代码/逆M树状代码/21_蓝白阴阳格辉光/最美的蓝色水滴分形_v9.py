#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""v9 - 叠加 UF1b 解析逆心形描边（原理4 策略B 第4.2节）
在 v8 基础上增加：1/(0.5*e^{it} - 0.25*e^{2it}) 精确数学边界 -> 锐利金线
金边辉光仍用距离估计 dn（外部势能为连续逃逸带，Sobel 会误点亮全部等值线，故不用）
"""
...
WIDTH=3400;HEIGHT=int(WIDTH*(RE_MAX-RE_MIN)/(IM_MAX-IM_MIN))
...
def render(data,w,h):
    ...
    line=binary_dilation(line,np.ones((2,2)))        # 轻微膨胀 -> 2~3px 锐线
    if line.any():
        cr,cg,cb=GOLD_WHITE
...
if __name__=='__main__':
    ...
    print(f"=== v9 解析逆心形描边版 === {WIDTH}x{HEIGHT} iter={MAX_ITER}")
    ...
    out=os.path.join(out_dir,'最美的蓝色水滴分形_v9.png')
