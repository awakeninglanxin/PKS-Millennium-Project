#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""v10 - 导入 Divetoxx/Mandelbrot 仓库的核心抗锯齿技术：真·SSAA + RGB空间积分
学习来源: https://github.com/Divetoxx/Mandelbrot  (main.cpp + README)
迁移要点:
  * 仓库做法: 先算 raw = target*SSAA 的完整数据, 再对 SSAA*SSAA 块在 RGB 空间累加平均 -> 边缘锐利/无锯齿/无色彩噪声
  * 我们做法: compute() 在 raw 分辨率迭代; render() 在 raw 分辨率算出每个子像素完整颜色(金边/棋盘/内部/解析描边);
    downsample() 对块内 RGB 累加平均 -> 金边自然锐利、棋盘砖缝自然柔化, 比后处理高斯模糊优雅
  * 其余外观参数沿用 v9 (解析逆心形描边 + 蓝色LUT + 5层金边)
本地默认 SSAA=2 (验证); GPU 可 SSAA=8 影院级
"""
...
def generate_outline(n_pts=9000):
    ...
    c=0.5*np.exp(1j*t)-0.25*np.exp(2j*t)   # 正M主心形精确边界
    inv=1.0/c                              # 复反演 -> 逆M水滴主轮廓
    return inv.real, inv.imag

def render(data,w,h):
    """在 w,h (raw 分辨率) 上算出每个子像素的完整颜色"""
    ...
    col=np.round((a-RE_MIN)/(RE_MAX-RE_MIN)*(w-1)).astype(int)
    row=np.round((b-IM_MIN)/(IM_MAX-IM_MIN)*(h-1)).astype(int)
    ok=(row>=1)&(row<h-1)&(col>=1)&(col<w-1)
    row,col=row[ok],col[ok]
    line=np.zeros((h,w),dtype=bool);line[row,col]=True
    line=binary_dilation(line,np.ones((2,2)))
    if line.any():
        cr,cg,cb=GOLD_WHITE
        img[line,0]=np.clip(img[line,0]*0.20+cr*0.80,0,1)
        img[line,1]=np.clip(img[line,1]*0.20+cg*0.80,0,1)
        img[line,2]=np.clip(img[line,2]*0.20+cb*0.80,0,1)
    return np.rot90(img,k=3)

def downsample(raw, ssaa):
    """真·SSAA: 对 ssaa*ssaa 块在 RGB 空间累加平均 (仓库核心做法)"""
    h,w,_=raw.shape
    H,W=h//ssaa,w//ssaa
    out=np.zeros((H,W,3),dtype=np.float64)
    for j in range(ssaa):
        for i in range(ssaa):
            out+=raw[j::ssaa, i::ssaa, :]
    out/=(ssaa*ssaa)
...
if __name__=='__main__':
    ...
    print(f"=== v10 SSAA版 === target={WIDTH}x{HEIGHT} SSAA={SSAA} raw={rw}x{rh} iter={MAX_ITER}")
    ...
    out=os.path.join(out_dir,f'最美的蓝色水滴分形_v10.png')
    fw=8;fh=fw*HEIGHT/WIDTH;fig,ax=plt.subplots(figsize=(fw,fh),dpi=150)
