...
# n_vs = [10,9,8,7,6,4,5,3]  # 定义参数列表 n_vs
n_vs = [6,5,3]
n_values = [int(value * 1) for value in n_vs]  # 将 n_vs 中的每个值乘以 1 并存储在 n_values 中
slices_values = [4] * len(n_vs)  # 定义 slices_values 列表，每个元素为 6
...
# alpha_v = 1.5*np.pi # 定义 alpha_v 参数
alpha_v = 0
filename = f'calabi-Yau超美3d_花_{len(n_vs)}图合一{alpha_v}_2^n.obj'  # 定义输出文件名

def calabi_yau(z, k1, k2, alpha, n):
    exp_k1 = np.exp(np.log(2)*2 * np.pi * (1j) * k1 / n) *(1-np.sin(z))# 计算 exp_k1
    exp_k2 = np.exp(np.log(2)*2 * np.pi * (1j) * k2 / n) *(1-np.sin(z))# 计算 exp_k2
    z1 = exp_k1 * (np.cosh(z) ** (2 / n))  # 计算 z1
    z2 = exp_k2 * (np.sinh(z) ** (2 / n))  # 计算 z2
    return np.real(z1), np.real(z2), abs(np.cos(alpha) * np.imag(z1) + np.sin(alpha) * np.imag(z2))  # 返回 z1 和 z2 的实部，以及 z1 和 z2 的虚部的组合
