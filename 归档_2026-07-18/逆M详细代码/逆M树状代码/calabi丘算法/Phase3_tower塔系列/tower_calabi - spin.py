...
alpha_v = np.pi
filename = f'calabi-Yau——spin3d图 正_{len(n_vs)}图合一{alpha_v}_2^n.obj'


def calabi_yau(z, k1, k2, alpha, n):
    exp_k1 = np.exp(2 * np.pi * (1j) * k1 / n)
    exp_k2 = np.exp(2 * np.pi * (1j) * k2 / n)
    z1 = exp_k1 * (np.tan(z) ** (2 / n))
    z2 = exp_k2 * (np.tan(z) ** (2 / n))
...
def create_cylinder_with_area(target_area, height, position=(0, 0, 0)):
    ...
    # Rotate cylinder to be vertical in the xz plane
