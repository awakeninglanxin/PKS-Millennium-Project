...
n_vs = [18, 20]  # 定义参数列表 n_vs
n_values = [int(value * 1) for value in n_vs]  # 将 n_vs 中的每个值乘以 2 并存储在 n_values 中
slices_values = [4] * len(n_vs)  # 定义 slices_values 列表，每个元素为 5
# scale_factors = list(range(len(n_vs)-1, -1, -1))  # 定义从 9 到 0 的 scale_factors 列表
# scale_factors = [2**factor for factor in scale_factors]  # 将 scale_factors 中的每个因子计算为 8 的次方
scale_factors= [1] * len(n_vs)
alpha_v = np.pi/2  # 定义 alpha_v 参数
filename = f'3d卡丘_{n_vs,len(n_vs)}图合一{alpha_v}_双色渐变(-).obj'  # 定义输出文件名
...
def calabi_yau(z, k1, k2, alpha, n):
    ...
    return np.real(z1), np.real(z2), -np.cos(alpha) * np.imag(z1) - np.sin(alpha) * np.imag(z2)  # 返回 z1 和 z2 的实部，以及 z1 和 z2 的虚部的组合
        ...
        for k2 in range(n):  # 遍历 k2 从 0 到 n-1
            for i in range(slices - 1):  # 遍历 i 从 0 到 slices-2
                for j in range(slices - 1):  # 遍历 j 从 0 到 slices-2
                    ...
                    color_z1 = cm.get_cmap('viridis')(np.interp(np.mean(z1_normalized), [-1, 1], [0, 1]))
                    color_z2 = cm.get_cmap('hsv_r')(np.interp(np.mean(z2_normalized), [-1, 1], [0, 1]))
                    combined_color = (np.array(color_z1[:3]) + np.array(color_z2[:3])) / 2  # 平均两个颜色
...
# Create all meshes with different scales
positions = [(0, 0, 0)] * len(n_vs)  # 定义位置列表，所有图形的中心重叠
all_meshes = [create_mesh(n, slices, pos, scale) for n, slices, pos, scale in zip(n_values, slices_values, positions, scale_factors)]  # 创建所有网格
