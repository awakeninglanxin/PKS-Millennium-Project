...
def av(i):
    ...
    ax.set_title(f'Inverse-M Droplet v12b Re~{Re} t={tp} [sway x3.5 12fps]',fontsize=12,fontweight='bold')
    ax.set_xlim(0,NX); ax.set_ylim(0,NY)
ani=animation.FuncAnimation(fig,av,frames=len(frames),interval=80)
g1=os.path.join(OUT_DIR,'droplet_v12b_vorticity.gif')
...
def av2(i):
    ...
    ax2.set_title(f'Inverse-M Vortex v12b Re~{Re} f={i} [sway x3.5 12fps]',fontsize=12)
    ax2.set_xlim(0,NX); ax2.set_ylim(0,NY)
ani2=animation.FuncAnimation(fig2,av2,frames=len(frames),interval=80)
g2=os.path.join(OUT_DIR,'droplet_v12b_vortex.gif')
