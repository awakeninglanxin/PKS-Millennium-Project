...
# -*- coding: utf-8 -*-
"""
Smart Python File Splitter v1.0
...
class PythonFileSplitter:
    ...
    def _update_index(self):
        """Generate or update CODE_INDEX.md for the project directory."""
        project_dir = str(self.input_file.parent.resolve())
        index_script = Path(__file__).parent / "generate_index.py"
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, str(index_script), project_dir],
                capture_output=True, text=True, encoding="utf-8"
            )
            if result.returncode == 0:
                print(f"\n📑 CODE_INDEX.md updated: {project_dir}/CODE_INDEX.md")
            else:
                print(f"\n⚠️  Index generation warning: {result.stderr[:200]}")
        except Exception as e:
            print(f"\n⚠️  Could not update CODE_INDEX.md: {e}")
    ...
    def _generate_main_file(self, tree: ast.AST, source_code: str):
        """生成主入口文件 - 写入模块目录内，不覆盖原文件"""
        # 主入口文件放在模块目录内（不覆盖原文件！）
        main_file = self.output_dir / f"{self.input_file.stem}_main.py"
        
        # 提取 if __name__ == "__main__": 代码块
        main_block_code = self._extract_main_block(tree, source_code)
        
        # 解析 main_block 得到内部的具体语句（去掉外层的 if __name__ 包装）
        main_body_lines = []
        if main_block_code:
            try:
                main_tree = ast.parse(main_block_code)
                if_node = main_tree.body[0]
                if isinstance(if_node, ast.If):
                    for stmt in if_node.body:
                        line = ast.unparse(stmt) if hasattr(ast, 'unparse') else ''
                        if line:
...
# -*- coding: utf-8 -*-
\"\"\"
{self.input_file.name} - 自动生成的主入口文件
由 python-file-splitter skill 生成
...
"""
        ...
        # 生成模块导入语句
        module_dir_name = self.output_dir.name
        for name, _, mod_path in self.modules:
            content += f"from {module_dir_name}.{mod_path.stem} import *\n"
        ...
        if main_body_lines:
            ...
            # 回退：没有找到 main 块，生成提示
            content += "    # ⚠️ 未检测到原文件的 if __name__ == '__main__' 执行代码\n"
            content += "    # 请手动补充执行逻辑，或检查原文件\n"
            content += "    print('请从此入口文件调用拆分后的模块')\n"
        
        with open(main_file, 'w', encoding='utf-8') as f:
            f.write(content)
        
        print(f"\n✅ 生成主入口文件: {main_file} (含原始执行逻辑)")
    ...
    def _print_report(self, original_size: int, violations: List[Tuple[str, int]]):
        ...
        print(f"🎉 拆分完成!")
        print(f"\n使用方式:")
        print(f"  python {self.input_file.stem}_main.py")
        print(f"{'='*60}")
