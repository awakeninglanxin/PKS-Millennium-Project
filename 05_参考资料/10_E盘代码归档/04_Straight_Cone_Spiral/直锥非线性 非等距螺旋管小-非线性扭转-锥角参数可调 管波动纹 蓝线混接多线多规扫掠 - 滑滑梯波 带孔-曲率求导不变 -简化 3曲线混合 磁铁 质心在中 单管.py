...
class SpiralSweepGenerator:
    def __init__(self, start_dxf_path, end_dxf_path, bottom=300, high=300, n_turns=1.5,
                 ...
                 amp_nonlinear=False, dxf_points=19, twist_type='linear', bands=5, use_parallel=True,
                 phase_dif=0, k=0.2):
        """初始化螺旋扫掠生成器 - 简化版本，去除middle断面"""
        # 固定参数设置
        self.bands = bands  # 固定为15波段
        self.phase_dif = phase_dif  # 固定相位差为0
    ...
    def update_psi(self):
        ...
        shift_neg1to1 = 0.6 #数值递减就是底看顺时针转

        if self.twist_type == 'linear':
            self.psi = np.linspace(-shift_neg1to1 * self._twist * t_length,
                                   (1 - shift_neg1to1) * self._twist * t_length,
                                   self.num_t)
            ...
            base_angle = -shift_neg1to1 * self._twist * t_length
    ...
    def _precompute_exp_wave(self):
        ...
        print(f"计算得到的k值: {self.k:.6f}, 目标波段数: {self.bands}, 实际波段数: {actual_bands:.2f}")
    ...
    def segment_egghead_points_by_jump(self, points):
        """根据锯齿波跳变点分割点曲线，只在段间添加混接曲线，并从中收集跳变点"""
        segments = []  # 存储分割后的点段
        current_segment = [points[0]]
        jump_points = []  # 存储混接曲线中的B4点

        # 改进的跳变点检测逻辑 - 使用相对阈值而不是固定阈值
        if len(points) < 2:
            return [points], jump_points

        # 计算波形值的变化率作为检测依据
        wave_changes = []
        for i in range(1, len(self.exp_wave_values)):
            change = abs(self.exp_wave_values[i] - self.exp_wave_values[i - 1])
            wave_changes.append(change)

        # 使用变化率的中位数作为基准阈值
        if wave_changes:
            median_change = np.median(wave_changes)
            # 动态阈值：中位数的3倍，但不超过最大变化率的50%
            max_change = max(wave_changes)
            jump_threshold = min(median_change * 3, max_change * 0.5)
        else:
            jump_threshold = 0.1  # 默认阈值

        print(f"跳变点检测阈值: {jump_threshold:.6f}")
        ...
        for i in range(len(points)):
            ...
            if i < len(points) - 1:
                # 使用改进的跳变检测逻辑
                if i < len(self.exp_wave_values) - 1:
                    wave_change = abs(self.exp_wave_values[i + 1] - self.exp_wave_values[i])

                    # 检测跳变：变化率超过阈值，且不是最后一个跳变点
                    if wave_change > jump_threshold and i < len(points) - 2:
                        # 额外的验证：确保这是真正的跳变而不是噪声
                        if (i > 0 and i < len(self.exp_wave_values) - 2 and
                                (self.exp_wave_values[i + 1] > self.exp_wave_values[i] + jump_threshold / 2 or
                                 self.exp_wave_values[i + 1] < self.exp_wave_values[i] - jump_threshold / 2)):
                            discrete_points.append(None)  # 标记跳变点
                            print(
                                f"在点 {i} 检测到跳变，波形值: {self.exp_wave_values[i]:.3f} -> {self.exp_wave_values[i + 1]:.3f}")
        ...
        for i in range(len(segments)):
            ...
            if i < len(segments) - 1:
                ...
                if len(prev_seg) >= 4 and len(next_seg) >= 5:
                    ...
                    print(f"在第 {i} 和 {i + 1} 段之间添加混接曲线，收集B4点")
                else:
                    print(f"警告: 段{i}或{i + 1}点数不足，无法创建G4混接曲线")
                    # 添加简单线性过渡作为后备方案
                    blend_points = [segments[i][-1], segments[i + 1][0]]
                    blended_segments.append(blend_points)
                    # 注意：线性过渡中没有B4点，所以不收集跳变点

        print(f"点曲线被分成 {len(blended_segments)} 段")
        print(f"从混接曲线中收集到 {len(jump_points)} 个B4跳变点")

        # 调试信息：显示各段的点数
        for i, seg in enumerate(blended_segments):
            print(f"段 {i}: {len(seg)} 个点")

        return blended_segments, jump_points  # 返回分段和跳变点列表（只包含B4点）
    ...
    def save_to_dxf(self, filename, bands_list=None):
        ...
        colors = {
            'sections': 3,  # 绿色 - 扫掠断面
            'axis': 8,  # 黑色 - 中心轴
            'end_section_lines': 1,  # 红色 - 原始投影线
            'new_end_section_lines': 9,  # 灰色 - 新增投影线
            'jump_points': 6,  # 紫色 - B4跳变点
            'circles': 4,  # 青色 - 同心圆
            'last_center_points': 5,  # 蓝色 - 最后中心点
            'guide_lines': 7  # 白色 - 导轨线
        }
        ...
        for j, points in enumerate(guide_lines.values()):
            ...
            for seg in segments:
                if len(seg) >= 2:
                    ...
                    spline.dxf.color = colors['guide_lines']
        ...
        filenamewhole = f'波纹螺旋锥管_{self.bands}段波.dxf'
        doc.saveas(filenamewhole)
        print(f"成功保存到 {filenamewhole}")
        print(f"保留了 {len(all_jump_points)} 个紫色跳变点(B4点)")
        print(f"保留了 {len(last_center_points)} 个最后中心点")

# 主程序也需要相应修改
if __name__ == "__main__":
    # 简化主程序，只使用两个断面文件
    curve_generator = SpiralSweepGenerator(
        'start 舒伯格蛋咬一口曲线.dxf', 'start 舒伯格蛋咬一口曲线.dxf',
        bottom=56,
        high=56*silver_ratio,
        amp=3.5,
        alpha_deg=6.7315,
        dxf_points=19,
    ...
    curve_generator.save_to_dxf('波纹螺旋锥管')
