...
class SpiralSweepGenerator:
    def __init__(self, spline_data_path, t_min=5* np.pi, t_max=11 * np.pi, d=2.5, z_v=10, amp=1, sign_spiral_dir=1, sign=1, twist=1):
        """
    ...
    def update_psi(self):
        """更新psi值"""
        self.psi = np.linspace(0, self._twist * self.t_length, self.num_t)
    ...
    def calculate_spiral_coords(self):
        ...
        z = (self.t_spiral- self.t_min) *self.z_v*np.exp(self.b * self.t_spiral) ##这是直螺旋
        return x, y, z
...
if __name__ == "__main__":
    ...
    generator_right.r = -1
    generator_right.twist = -0.5 # 黄金比的周期
