...
def extract_spline_from_dxf(dxf_path, num_points=16):
    """
    从DXF文件中提取SPLINE曲线数据，并将参数范围映射到0-2π。

    Args:
        dxf_path (str): DXF文件的路径
        num_points (int): 采样点数量，默认16
...
if __name__ == "__main__":
    ...
    parser.add_argument('dxf_path', nargs='?', default="your_curve口朝右下.dxf",
                        help='DXF文件路径')
    parser.add_argument('--output', '-o', default="spline_points.csv",
                        help='输出CSV文件路径')
    parser.add_argument('--points', '-n', type=int, default=64,
                        help='采样点数量')
